...
/*RWD 3:2000*/
static long     peakchunk_offset = 0;
...
/* RWD.2.98 */
	...
	/* RWD.2.98 have to do these later */
        ...
        if (peakchunks) {
          ...
          if ((peakchunk_offset = lseek(fd,0,SEEK_CUR)) < 0)
            die(Str(X_743,"error writing WAV header"));
          peakdata.ckID = *(long *) PEAK_ID;
          peakdata.chunkDataSize = sizeof(PeakChunk) + (nchnls-1)*sizeof(PositionPeak);
          peakdata.version = lenlong(1);
          peakdata.timeStamp = lenlong(0); /*dummy time to start with */
          if(write(fd, (char *)&peakdata, sizeof(PeakChunk)-sizeof(PositionPeak)) < 0)
            die(Str(X_743,"error writing WAV header"));
...
{                                 /*        & optionally under -R        */
    ...
    if (peakchunks) {
      ...
      if ((lseek(fd,peakchunk_offset,0) < 0) || write_wavpeak(fd, verbose))
        die(Str(X_729,"error rewriting WAV header"));
    }
    lseek(fd, endpos, 0);
}
...
/*                printf("Found smpl chunk\n"); */
 ...
 /*RWD 3:2000 NB changed: chunk read must be called each pass */
            else if (lseek(fd,natllong(chunksize),SEEK_CUR) < 0)
              die(Str(X_732,"error skipping unknown chunk in WAV file"));
            if(read(fd,(void*)&chunk_id,sizeof(long)) < 0
                || read(fd,(void*)&chunksize,sizeof(long)) < 0
...
#ifdef _macintosh
void wavResetFrameSize(int sampsize, int nchnls) {
    framesize = sampsize * nchnls;
...
static int write_wavpeak(int fd, int verbose)
{
    ...
    data.chunkDataSize = sizeof(PeakChunk) + (nchnls-1)*sizeof(PositionPeak);
	/*RWD 3:2000 chunk size excludes tag and size fields*/
    data.chunkDataSize -= (2 * sizeof(long));
    data.version = lenlong(1);
    data.timeStamp = lenlong(time(NULL));
    write(fd, (char *)&data, sizeof(PeakChunk)-sizeof(PositionPeak));
    for (i=0; i<nchnls; i++) {
      /*RWD 3:2000 peak vals are always normalized */
      data.peak[0].value = (float)omaxamp[i]/32768.0f;
