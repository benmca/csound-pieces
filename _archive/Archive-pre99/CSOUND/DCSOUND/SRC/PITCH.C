...
void Foscaa(OSC *p)
{
	...
	ftp = p->ftp;
	if (ftp==NULL) {
          initerror(Str(X_1106,"oscil: not initialized"));
          return;
	}
	ftbl = ftp->ftable;
	len = ftp->flen;
...
void trnseg(TRANSEG *p)
{
    ...
    if (p->auxch.auxp==NULL) {
      initerror(Str(X_1553, "transeg: not initialized (arate)\n"));
    }
...
{
    if (*p->len == FL(0.0)) 
      p->length = 1 + (int)(p->h.insdshead->p3 * esr * 0.5);
    else
      p->length = 1 + (int)*p->len;
    if (p->length <= 1) p->length = esr;
