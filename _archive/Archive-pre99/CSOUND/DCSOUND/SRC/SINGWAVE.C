...
void make_Modulatr(Modulatr *p, MYFLT *i)
{
    FUNC	*ftp;

    if ((ftp = ftfind(i)) != NULL)	p->wave = ftp;
    else perferror(Str(X_382,"No table for Modulatr")); /* Expect sine wave */
    p->v_time = FL(0.0);
/*     p->v_rate = 6.0; */
/*     p->vibAmt = 0.04; */
...
void Modulatr_print(Modulatr *p)
{
    printf("Modulatr: v_rate=%f v_time=%f vibAmt=%f\n",
           p->v_rate, p->v_time, p->vibAmt);
}

void make_SingWave(SingWave *p, MYFLT *ifn, MYFLT *ivfn)
{
    FUNC	*ftp;

    if ((ftp = ftfind(ifn)) != NULL) p->wave = ftp;
    else perferror(Str(X_383,"No table for Singwave")); 
    p->mytime = FL(0.0);
    p->rate = FL(1.0);
...
void SingWave_setFreq(SingWave *p, MYFLT aFreq)
{
    ...
    p->rate = (MYFLT)p->wave->flen * aFreq / esr;
