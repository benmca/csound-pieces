...
#if defined(_macintosh) || defined(SYMANTECS)
    p = strrchr(name_full, ':');
#else
    p = strrchr(name_full, '/');
...
void rdorchfile(void)		/* read entire orch file into txt space */
{
	...
	if (fseek(fp, 0L, SEEK_END) != 0)
		dies(Str(X_626,"can't find end of file %s"),orchname);
	...
	while ((c = getorchar()) != EOF) {	/* read entire orch file  */
		...
		*cp++ = c;
		...
		if (c == '\\') {	/* Continuation?? */
			...
			(d=getc(fp),
			ungetc(d,fp)) == '*') 	{  /* commented macros with c-style (gab A9)*/
			do {
				c= getc(fp);
				d= getc(fp);
				ungetc(d,fp);
		  ...
		  if (mm->acnt) {
			  ...
			  for (j=0; j<mm->acnt; j++) {
				  ...
				  while ((c = getorchar())!= term && c!=trm1) {
					  char d;  /* gab B0 to allow comments in macro call argments*/
					  if (c==';') {
						  while ((c=getorchar())!='\n') ;
					  }
					  else if (c=='/' && (d=getorchar())=='*') {
						  do {
							  c= getc(fp);
							  d= getc(fp);
							  ungetc(d,fp);
					  ...
					  if (/*c ==' ' ||*/ c== '\n') {
						  srccnt++;
						  continue; /* gab B0 to allow line changes in macro call argments*/
					  }
					  if (c == '(' ) {  /*nested macro arguments */
						  do {
							  nn->body[i++] = c;
		...
		{
			void write_expanded_orc(char *);
			write_expanded_orc(ortext); /* gab A9*/
		}

}

void write_expanded_orc(char *ortext) /* gab A9*/
{
	FILE * f;
	char *p=ortext;
	if ((f=fopen("orc.srt","wa"))== NULL) die("error opening orc.srt");
	while(*p != '\0' && *p != EOF && *p != 'Í'){
		putc(*p,f);
		p++;
	}
...
/*                 printf("%%%%%%%%%%newline in comment\n"); */
		...
		*cp++ = '\0';			/*  terminate strng */
	    ...
	    if ( isalnum(c)	    /* establish validity */
		 || c == '+' || c == '-'
		 || c == '*' || c == '/'
		 || c == '%' || c == '^'
                 || c == '.' || c == '_'
...
tstnxt:
    ...
    if (nxtest < opgrpno-1) {
		...
		if (strcmp(linopcod,"=") == 0
			...
			strcmp(linopcod,"tab") == 0 ||   /* gab c3*/
			strcmp(linopcod,"tabw") == 0 ||  /* gab c3*/
...
chkin:
			...
			while (n--) {			/* inargs:   */
				...
				if (tfound != 'c' && tfound != 'p'
					&& tfound != 'S' && !lgprevdef
					&& !(
					   !strcmp(tp->opcod, "rtrnca") || !strcmp(tp->opcod, "parmta")	 /* gab-a7 fixed */
					|| !strcmp(tp->opcod, "rtrnck") || !strcmp(tp->opcod, "parmtk") 
					|| !strcmp(tp->opcod, "fin")    || !strcmp(tp->opcod, "fink")  
					|| !strcmp(tp->opcod, "fini") 
					|| !strcmp(tp->opcod, "trigseq")|| !strcmp(tp->opcod, "vtablek") 
					|| !strcmp(tp->opcod, "vtablei") || !strcmp(tp->opcod, "vtablea")
					|| !strcmp(tp->opcod, "vtabi") || !strcmp(tp->opcod, "vtabk")
...
static void intyperr(int n, char tfound)
{
	...
	sprintf(errmsg,Str(X_913,"input arg '%s' of type %s not allowed"),s,t);
	synterr(errmsg);
}
