...
void AWopen(nchnls, srate, oMaxLag)       /* open for audio output */
  int nchnls, oMaxLag;
  long srate;
{
extern  char  errmsg[];
	...
	Attribs.attr.sampled_attr.channels = nchnls;
  ...
  /* create an audio stream  */
	xid = APlaySStream( audio, ~0, &PlayAttribs, &streamParams, &audioStream, NULL);
  ...
  /* start stream paused so we can transfer enough data (2 second worth) before
	...
	if (oMaxLag < audio->block_size) {
	    sprintf(errmsg, "-B %d probably too small, suggest %d",
		    oMaxLag, audio->block_size);
	    warning(errmsg);
	}
}

void AudioWrite(outbuf, nbytes)                      /* put samples to DAC  */
  char *outbuf;
  int  nbytes;
{
	if (write(streamSocket, outbuf, nbytes) < nbytes)
	    printf("HP write to streamSocket: couldn't write all bytes requested\n");
	if (audioPaused) {
	    pauseCount -= nbytes;
	    if (pauseCount <= 0) {
