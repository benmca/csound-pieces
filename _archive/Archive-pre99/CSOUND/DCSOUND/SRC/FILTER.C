 ...
 * ================================
 *
 * Since filter allows the implementation of generalized recursive filters,
...
void ifilter(FILTER* flt)
{
  ...
  if ((flt->numb<1) || (flt->numb>(MAXZEROS+1)) ||
    ...
    error(Str(X_278,"Filter order out of bounds: (1<=nb<51, 0<=na<=50)"),"<filter>");
...
void insertFilter(FILTER* flt, double val)
{
  ...
  *flt->currPos = val;
  ...
  if (((double*) (++flt->currPos)) > ((double*)flt->delay.auxp + (flt->ndelay-1)) )
...
void laguer(fcomplex a[], int m, fcomplex *x, int *its)
{
	...
	err = Cabs(b)+abx*err;
      ...
      dx = ((FPMAX(abp,abm) > 0.0 ? Cdiv(Complex((double) m,0.0),gp)
	   : RCmul((double)exp(log(1.0+abx)),Complex((double)cos((double)iter),(double)sin((double)iter)))));
      x1 = Csub(*x,dx);
