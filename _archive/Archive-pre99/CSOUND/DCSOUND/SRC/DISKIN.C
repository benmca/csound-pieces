...
static int sngetset(SOUNDINEW *p, char *sfname)
{
    ...
    if ((sinfd = openin(sfname)) < 0) {     /* open with full dir paths */
      if (isfullpath(sfname))
        ...
        sprintf(errmsg,Str(X_695,"diskin can't find \"%s\" in its search paths"),
                sfname);
      goto errtn;
    }
...
/* #####RWD: it is not safe to assume all compilers init this to 0 */
	...
	p->audrem = -1;
      ...
      if ((p->filepos =         /* seek to bndry */
           lseek(p->fdch.fd, nbytes+p->firstsampinfile, 0)) < 0)
	die(Str(X_698,"diskin seek error during reinit"));

      if ((n =                  /* now rd fulbuf */
           sreadinew(p->fdch.fd,p->inbuf,SNDINEWBUFSIZ,p)) == 0)
	p->endfile = 1;
	...
	p->audrem = p->audsize-nbytes+p->firstsampinfile;
      ...
      if ((p->filepos =         /* seek to bndry */
           lseek(sinfd, nbytes+p->firstsampinfile, 0)) < 0)
    ...
    if ((n =                    /* now rd fulbuf */
         sreadinew(sinfd,p->inbuf,SNDINEWBUFSIZ,p)) == 0)
      p->endfile = 1;
    p->inbufp = p->inbuf;
    p->bufend = p->inbuf + n;
...
void sndo1set(SNDOUT *p)            /* init routine for instr soundout   */
{
    ...
    if ((soutfd = openout(sfname, 1)) < 0) {   /* if openout successful */
      ...
      else sprintf(errmsg,Str(X_1211,"soundout can't find %s in search paths"), sfname);
      goto errtn;
    }
    sfname = retfilnam;
