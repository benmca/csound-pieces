...
{                                   /*      then run an init pass           */
    ...
    tp = instrtxtp[insno];
    if (tp->mdepends & 04) {
      printf(Str(X_925,"instr %d expects midi event data, can't run from score\n"),
	     insno);
      return(1);
    }
    ...
    /*  then run an init pass           */
{
 ...
 actlnk:
    ip->insno = insno;
	ip->p1 = (MYFLT) insno;   /* gab e4 */
	ip->p2 = kcounter/ ekr; /* gab e4 */
	ip->p0 = mep->chan+1.f;	/* gab e4 p0 contains the MIDI Channel */
    if (instrtxtp[insno]->active++ > instrtxtp[insno]->maxalloc &&
        instrtxtp[insno]->maxalloc>0) {
      instrtxtp[insno]->active--;
      warning(Str(X_27,"can't allocate last note because it exceeds instr maxalloc"));
...
{                                       /* called by turnoff statmt at Ptime */
    ...
    if (curip->xtratim) {                   /* if extra time needed:  */
      ...
      curip->relesing = 1;
      curip->offtim = newtim;
      if (oldtim < 0.)                    /* if indef duratn instr   */
                        ...
                        /*      & send audio result to output buffer    */
{
    ...
    INSDS  *ip;
    long    kreq = kcnt;
	extern int flag3d; /*gab c3*/
	int activeflag=0; /* gab c3 */
    VMSG(printf("perfing %ld kprds\n",kcnt);)
    if (!O.ksensing && actanchor.nxtact == NULL) {  /* if !kreads & !instrs_activ, */
      kcounter += kcnt;
#if defined(_macintosh) || defined(CWIN) || defined(SYMANTEC)
...
#elif defined(VST) /* gab e1 */
			{
				long pcnt = kcnt;
				extern  unsigned int outbufsiz;
				extern float *spout;
				extern int nspout;
				//memset(spout,0,outbufsiz);
				memset(spout,0,nspout*sizeof(float));
				do (*nzerotran)(1L);     /*   send kcnt zerospouts  */
				while (--pcnt);
      ...
      spoutactive = 0;        /*   make spout inactive   */
			if (flag3d) { /* gab c3*/
				extern int spoutactive3D[], spout3Dnum; 
				int j;
        ...
        while ((pds = pds->nxtp) != NULL) {
          (*pds->opadr)(pds); /*      run each opcode    */
			...
			if (flag3d) { /* gab c3*/
				...
				activeflag=0;
				for (j = 0; j < spout3Dnum; j++)
					activeflag |= spoutactive3D[j];
			}
      if (spoutactive || activeflag) /* gab c3*/       /*   results now in spout? */
