...
void ktable(TABLE  *p)
{
    ...
    ftp = p->ftp;
    if (ftp==NULL) {            /* RWD fix */
      initerror(Str(X_1264,"table(krate): not initialized"));
      return;
    }
    ndx = *p->xndx;
    length = ftp->flen;
...
void tablefn(TABLE  *p)
{
    ...
    ftp = p->ftp;
    if (ftp==NULL) {            /* RWD fix */
      initerror(Str(X_1267,"table: not initialized"));
      return;
    }
    rslt = p->rslt;
    length = ftp->flen;
...
void ktabli(TABLE  *p)
{
    ...
    ftp = p->ftp;
    if (ftp==NULL) {
      initerror(Str(X_1268,"tablei(krate): not initialized"));
      return;
    }
    ndx = *p->xndx;
    length = ftp->flen;
...
void ktabl3(TABLE  *p)
{
    ...
    ftp = p->ftp;
    if (ftp==NULL) {
      initerror(Str(X_1265,"table3(krate): not initialized"));
      return;
    }
    ndx = *p->xndx;
    length = ftp->flen;
...
void tabli(TABLE  *p)
{
    ...
    ftp = p->ftp;
    if (ftp==NULL) {
      initerror(Str(X_1269,"tablei: not initialized"));
      return;
    }
    rslt = p->rslt;
    length = ftp->flen;
...
void tabl3(TABLE  *p)           /* Like tabli but cubic interpolation */
{
    ...
    ftp = p->ftp;
    if (ftp==NULL) {
      initerror(Str(X_1266,"table3: not initialized"));
      return;
    }
    rslt = p->rslt;
    length = ftp->flen;
...
#pragma options(!global_optimizer)
	...
	ftp = p->ftp;
	if (ftp==NULL) {
          initerror(Str(X_1102,"oscil1(krate): not initialized"));
          return;
	}
	phs = p->phs;
...
void kosc1i(OSCIL1  *p)
{
    ...
    ftp = p->ftp;
    if (ftp==NULL) {
      initerror(Str(X_1103,"oscil1i(krate): not initialized"));
      return;
    }
    phs = p->phs;
    fract = PFRAC(phs); 	
...
void osciln(OSCILN *p)
{
    ...
    if (p->ftp==NULL) {
      initerror(Str(X_1109,"osciln: not initialized"));
      return;
    }     
    if (p->ntimes) {
...
#pragma options(!global_optimizer)
	...
	ftp = p->ftp;
	if (ftp==NULL) {
          initerror(Str(X_1101,"oscil(krate): not initialized"));
          return;
	}
	phs = p->lphs;
...
void koscli(OSC  *p)
{
	...
	ftp = p->ftp;
	if (ftp==NULL) {
          initerror(Str(X_1107,"oscili(krate): not initialized"));
          return;
	}
	fract = PFRAC(phs);
	ftab = ftp->ftable + (phs >> ftp->lobits);
...
void osckki(OSC  *p)
{
	...
	ftp = p->ftp;
	if (ftp==NULL) {
          initerror(Str(X_1108,"oscili: not initialized"));
          return;
	}
	lobits = ftp->lobits;
	phs = p->lphs;
...
void koscl3(OSC  *p)
{
	...
	ftp = p->ftp;
	if (ftp==NULL) {
          initerror(Str(X_1104,"oscil3(krate): not initialized"));
          return;
	}
	ftab = ftp->ftable;
	fract = PFRAC(phs);
...
void osckk3(OSC  *p)
{
	...
	ftp = p->ftp;
	if (ftp==NULL) {
          initerror(Str(X_1105,"oscil3: not initialized"));
          return;
	}
        ftab = ftp->ftable;
	lobits = ftp->lobits;
