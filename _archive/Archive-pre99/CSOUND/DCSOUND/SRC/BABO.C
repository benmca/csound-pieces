...
#else
#	define	INLINE  static 	/* compilers that do not understand 'inline' */
                                /* That is ANSI C CompilersA */
...
{
    size_t num_floats = (size_t) round((MYFLT)ceil((double)(max_time * esr)));
...
BaboMatrix_create_FDN(BaboMatrix *this, MYFLT diffusion)
{
    ...
    MYFLT  real_X[BABO_NODES]       = { .0 }, 
           imaginary_X[BABO_NODES]  = { .0 }, 
           arg_X[BABO_NODES]        = { .0 }, 
           real_x[BABO_NODES]       = { .0 };

    for(i = 0; i < BABO_NODES; ++i)
    { 
        real_X[i] = imaginary_X[i] = .0;
...
babo(void *entry)
{
    ...
    BaboTaplineParameters left = { 0 }, right = { 0 };
