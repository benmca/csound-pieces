...
void make_FM4Op(FM4OP *p)
{
    ...
    if ((ftp = ftfind(p->vifn)) != NULL) p->vibWave = ftp;
      ...
      perferror(Str(X_384,"No table for VibWaveato")); /* Expect sine wave */
      return;
    }
    p->baseFreq = FL(440.0);
    p->ratios[0] = FL(1.0);
...
static int FM4Op_loadWaves(FM4OP *p)
{
    ...
    if ((ftp = ftfind(p->ifn0)) != NULL) p->waves[0] = ftp;
      ...
      perferror(Str(X_377,"No table for FM4Op")); /* Expect sine wave */
      return 1;
    }
    if ((ftp = ftfind(p->ifn1)) != NULL) p->waves[1] = ftp;
    else {
      perferror(Str(X_373,"No table for  FM4Op")); /* Expect sine wave */
      return 1;
    }
...
void tubebellset(FM4OP *p)
{
    ...
    make_FM4Op(p);
    if (FM4Op_loadWaves(p)) return; /* 4 times  "rawwaves/sinewave.raw" */
...
/*      ADSR_setAll(&p->adsr[3], FL(0.04),0.00001f,FL(0.0),0.02f); */
    ...
    p->v_rate = FL(2.0) * p->vibWave->flen / esr; /* Vib rate */
                                /* Set Freq */
...
void rhodeset(FM4OP *p)
{
    ...
    make_FM4Op(p);
    if (FM4Op_loadWaves(p)) return; /* 3 times  "sinewave.raw"; 1 x fwavblnk.raw */
...
{
    make_FM4Op(p);
    if (FM4Op_loadWaves(p)) return;         /* Mixed -- 2 x sine; 1 x fwavblnk */
...
void b3set(FM4OP *p)
{
    ...
    make_FM4Op(p);
    if (FM4Op_loadWaves(p)) return;         /* sines */
...
MYFLT FM4Alg6_tick(FM4OPV *q)
{
    ...
    temp2 = (FL(1.0) + temp2) * p->baseFreq / esr;
...
void FMVoiceset(FM4OPV *q)
{
    ...
    ADSR_setAllTimes(&p->adsr[0], FL(0.050), FL(0.050), FM4Op_susLevels[15], FL(0.050));
    ADSR_setAllTimes(&p->adsr[1], FL(0.050), FL(0.050), FM4Op_susLevels[15], FL(0.050));
    ADSR_setAllTimes(&p->adsr[2], FL(0.050), FL(0.050), FM4Op_susLevels[15], FL(0.050));
    ADSR_setAllTimes(&p->adsr[3], FL(0.001), FL(0.010), FM4Op_susLevels[15], FL(0.500));
/*      ADSR_setAll(&p->adsr[0], FL(0.001), FL(0.001), FM4Op_susLevels[15], FL(0.001)); */
/*      ADSR_setAll(&p->adsr[1], FL(0.001), FL(0.001), FM4Op_susLevels[15], FL(0.001)); */
/*      ADSR_setAll(&p->adsr[2], FL(0.001), FL(0.001), FM4Op_susLevels[15], FL(0.001)); */
...
MYFLT FM4Alg4_tick(FM4OP *p, MYFLT c1, MYFLT c2)
{
    ...
    temp = Wave_tick(&p->v_time, (int)p->vibWave->flen,
                     p->vibWave->ftable, p->v_rate, FL(0.0)) *
      *p->modDepth * FL(0.2);
    temp = p-> baseFreq * (FL(1.0) + temp)/ esr;
...
void percfluteset(FM4OP *p)
{
    ...
    make_FM4Op(p);
    if (FM4Op_loadWaves(p)) return;         /* 3 x sines; 1 x fwavblnk */
    ...
    ADSR_setAllTimes(&p->adsr[0], FL(0.05), FL(0.05), FM4Op_susLevels[14], FL(0.05));
    ADSR_setAllTimes(&p->adsr[1], FL(0.02), FL(0.50), FM4Op_susLevels[13], FL(0.5));
    ADSR_setAllTimes(&p->adsr[2], FL(0.02), FL(0.30), FM4Op_susLevels[11], FL(0.05));
    ADSR_setAllTimes(&p->adsr[3], FL(0.02), FL(0.05), FM4Op_susLevels[13], FL(0.01));
