...
int main(int argc, char **argv)
{
    ...
    MYFLT
        ...
        ftot = FL(0.0),/* scale factor for calculating statistics */
...
/*             usage(0);    /\* then exits normally *\/ */
    ...
    if (nfile==NULL) {
      fprintf(stderr, "Must have an example noise file (-I name)\n");
      exit(1);
    }
...
/*     ofd = fopen(outfile, "wb"); */
    ...
    if ((fp = SAsndgetset(nfile,&pn,&beg_ntime,&input_ndur,&srn,channel))<0) {
      perror("dnoise: can't open noise reference file\n");
      exit(1);
    }
    ...
    for (i = 1; i < 4096; i *= 2)
      if (i >= N)
        break;
    if (i != N)
      fprintf(stderr,"dnoise: warning - N not a valid power of two; "
              "revised N = %d\n",i);
    N = i;
    N2 = N / 2;
    Np2 = N + 2;
    ...
    if (W != -1) {
      if (M != 0)
        fprintf(stderr,"dnoise: warning - don't specify both M and W\n");
      else
        if (W == 0)
          M = 4*N;
        else
          if (W == 1)
            ...
            if (W == 2)
              ...
              if (W == 3)
                ...
                fprintf(stderr,"dnoise: warning - invalid W ignored\n");
    }
    ...
    if (M < 7)
      fprintf(stderr,"dnoise: warning - M is too small\n");
    ...
    if (M <= N) {
      ...
      if (Leven)
        *sWin *= (MYFLT) I * sin(PI*0.5/(double) I) / (PI_F*FL(0.5));
      for (i = 1; i <= sLen; i++)
        sWin[i] *= I * (MYFLT)sin(PI*((double) i+0.5*(double) Leven)/(double) I)
...
/*     nread /= sizeof(MYFLT); */
    ...
    /* synthesis: The synthesis subroutine uses the Weighted Overlap-Add
      ...
      if (Verbose) {
        ...
        if (nImodR > (long) R) {
          nImodR -= (long) R;
          fprintf(stderr,"%5.1f seconds of input complete\n",(time+D*invR));
        }
      }
...
void hamming(MYFLT *win, int winLen, int even)
{
    ...
    ftmp = PI_F / (MYFLT) winLen;

    if (even) {
      for (i = 0; i < winLen; i++)
        win[i] = 0.54f + 0.46f * (MYFLT) cos((double) ftmp * ((double)i + 0.5));
      win[winLen] = FL(0.0);
    }
    else {
      *win = FL(1.0);
      for (i = 1; i <= winLen; i++)
        win[i] = FL(0.54) + FL(0.46) * (MYFLT) cos((double) ftmp * (double) i);
    }
