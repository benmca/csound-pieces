...
void	spat3d_init_window (SPAT3D *p)
{
	int	i, j, o;
	double	d, w;
...
void	spat3d_init_eq (SPAT3D_WALL *wstruct, MYFLT *ftable)
{
	int	eqmode;
	double	omega, k, kk, vk, vkk, vkdq, sq;

	/* EQ code taken from biquad.c */

	eqmode = (int) ((double) ftable[3] + 0.5);		/* mode	     */
	omega = TWOPI * (double) ftable[0] / (double) esr;	/* frequency */
	...
	if (eqmode >= 1) {
		wstruct->b0 = (MYFLT) (1.0 + sq * k + vkk);
		wstruct->b1 = (MYFLT) (2.0 * (vkk - 1.0));
		wstruct->b2 = (MYFLT) (1.0 - sq * k + vkk);
	} else {
		wstruct->b0 = (MYFLT) (1.0 + vkdq + kk);
		wstruct->b1 = (MYFLT) (2.0 * (kk - 1.0));
		wstruct->b2 = (MYFLT) (1.0 - vkdq + kk);
	}
	wstruct->a0 = (MYFLT) (1.0 + (k / (double) ftable[2]) + kk);
	wstruct->a1 = (MYFLT) (2.0 * (kk - 1.0));
	wstruct->a2 = (MYFLT) (1.0 - (k / (double) ftable[2]) + kk);
	if (eqmode > 1) {
		wstruct->a1 = -(wstruct->a1);
		wstruct->b1 = -(wstruct->b1);
	}
	wstruct->a0 = FL(1.0) / wstruct->a0;
}

/* initialise wall structures */

SPAT3D_WALL*	spat3d_init_wall (
...
void	spat3d_init_delay (SPAT3D *p)
{
	long	i, j;
...
void	spat3d_set_opcode_params (SPAT3D *p)
{
...
void	spat3dset (SPAT3D *p)
{
	long	wmax;

	if (*(p->args[13]) != FL(0.0)) return;	/* skip init	*/
	p->o_num = 1;				/* opcode number	 */
	spat3d_set_opcode_params (p);		/* set parameters	 */
	if (p->maxdep < 0) return;		/* nothing to render */
...
void	spat3diset (SPAT3D *p)
{
	long	wmax;

	if (*(p->args[11]) != FL(0.0)) return;	/* skip init	*/
	...
	MYFLT		Z)		/* sound source Z coordinate	*/
{
	...
	nn = p->bs; while (nn--) {	/* EQ */
		...
		*yn = ws->b2 * ws->xnm2 + ws->b1 * (ws->xnm2 = ws->xnm1);
		*yn += ws->b0 * (ws->xnm1 = *(xn++) * ws->scl);
		*yn -= ws->a2 * ws->ynm2 + ws->a1 * (ws->ynm2 = ws->ynm1);
		ws->ynm1 = (*yn *= ws->a0);
...
void	spat3d (SPAT3D *p)
{
	...
	if (p->maxdep < 0) return;	/* depth < 0 : nothing to render */

	if ((p->ws.auxp == NULL) || (p->y.auxp == NULL)) {
		initerror (Str(X_1558,"spat3d: not initialised")); return;
	}
...
void	spat3di (SPAT3D *p)
{
	...
	if ((p->ws.auxp == NULL) || (p->y.auxp == NULL)) {
		initerror (Str(X_1559,"spat3di: not initialised")); return;
	}
...
void	spat3dt (SPAT3D *p)
{
	...
	if ((p->outft == NULL) || (p->outftlnth < 4)) return;	/* no table */
