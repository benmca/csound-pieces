 ...
 * (2)  floating-point WAVE and AIFF-C files should contain normalised floats (+- 1.0)
 ...
 * (5)  PEAK data IN ALL CASES is normalised to +- 1.0. For floatsam files, where the values can
...
#endif
/* RWD.2.98 WAVE floats format must be NORMALISED +- 1 */
...
/*RWD 5:2001 */
static void audwrtrev3(char *outbuf, int nbytes) /* diskfile write option for audtran's */
                                /*      assigned during sfopenout()    */
{
...
static void audwrtrev4(char *outbuf, int nbytes) /* diskfile write option for audtran's */
                                /*      assigned during sfopenout()    */
{
	/*RWD 5:2001 removed format fixups! Now in audtran funcs, so good for rtaudio too */
...
{                               /*    called only if -i flag   */
    ...
    if (O.infilename != NULL && strcmp(O.infilename,"stdin") == 0) {
      ...
      if (strncmp(O.infilename,"devaudio", 8) == 0)
        sscanf(O.infilename+8, "%d", &win_dev);
...
# endif
      ...
      if (strncmp(O.outfilename,"devaudio", 8) == 0)
        sscanf(O.outfilename+8, "%d", &win_dev);
      else if (strncmp(O.outfilename,"dac", 3) == 0)
        sscanf(O.outfilename+3, "%d", &win_dev);
#endif
      sfoutname = O.outfilename;
...
{                                       /*      write buffer when full      */
 ...
 nchk:
    ...
    do {
      ...
      longsmp = (long) *sp;
	  longsmp <<= OUTLONGFAC;		/*RWD 5:2001 moved here so good for rtaudio out*/
      if (osfopen)
        *lloutbufp++ = longsmp;
      if (multichan) {
	maxps++;
...
{                                       /*      write buffer when full      */
 ...
 nchk:
    ...
    do {
      ...
      fltsmp = (float)(*sp * (float)OUTFLOATFAC);	 /*RWD 5:2001 */
      if (osfopen)
        *floutbufp++ = fltsmp;
      if (multichan) {
	maxps++;
...
static void int24ptran(void)
{
 ...
 nchk:
    ...
    do {
      ...
      s24.lsamp = (long) (*sp * OUT24LONGFAC);

      if (osfopen) {
        /* this is for direct 24bit packed output */
#if defined(_macintosh) || defined(SGI) || defined(LINUX_BE)
...
static void longrecv(void)              /* get spin values from long_int inbuf */
{
 ...
 nchk:
    ...
    inbufrem -= n;
/* --------------------------- IV (Nov 05 2001) ---------------------------> */
    do *r++ = (MYFLT) *llinbufp++ * (MYFLT) INLONGFAC;
/* <-------------------------- IV (Nov 05 2001) ---------------------------- */
    while (--n);
    if (!inbufrem) {
    echk:
...
static void floatrecv(void)              /* get spin values from float inbuf */
{
 ...
 nchk:
    ...
    inbufrem -= n;
/* --------------------------- IV (Nov 05 2001) ---------------------------> */
    do *r++ = (MYFLT) *flinbufp++ * (MYFLT) INMYFLTFAC;
/* <-------------------------- IV (Nov 05 2001) ---------------------------- */
    while (--n);
    if (!inbufrem) {
    echk:
