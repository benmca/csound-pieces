...
/* --------------------------- IV (Nov 05 2001) ---------------------------> */
MYFLT	esr = FL(-1.0), ekr = FL(-1.0), ensmps = FL(-1.0);
int	nchnls = DFLT_NCHNLS, ksmps = -1;
/* <-------------------------- IV (Nov 05 2001) ---------------------------- */
...
/* --------------------------- IV (Nov 05 2001) ---------------------------> */
    if (ensmps == FL(-1.0)) {		/* set default sr, kr, and ksmps */
      if (esr == FL(-1.0)) {			/* fix by Istvan Varga	*/
        if (ekr == FL(-1.0)) ekr = DFLT_KR;	/* Oct 2001		*/
        ksmps = (int) (ensmps = (esr = DFLT_SR) / ekr);
      } else {
        if (ekr == FL(-1.0)) {
          ksmps = (int) (ensmps = (MYFLT) DFLT_KSMPS); ekr = esr / ensmps;
        } else {
          ksmps = (int) (ensmps = esr / ekr);
        }
      }
    } else {
      if ((esr == FL(-1.0)) && (ekr == FL(-1.0))) {
        esr = DFLT_SR; ekr = esr / ensmps;
      } else {
        if (esr == FL(-1.0)) {
          esr = ekr * ensmps;
        } else if (ekr == FL(-1.0)) {
    ...
    VMSG( printf("esr = %7.1f, ekr = %7.1f, ksmps = %g, nchnls = %d\n",
		 esr ,ekr, ensmps, nchnls); ) ; 
    if (O.sr_override) {	/* if command-line overrides, apply now */
      esr = (MYFLT)O.sr_override;
      ekr = (MYFLT)O.kr_override;
      ksmps = (int) (ensmps = esr / ekr);
      printf(Str(X_1173,"sample rate overrides: esr = %7.1f, ekr = %7.1f, ksmps = %g\n"),
	     esr, ekr, ensmps);
    }
/* <-------------------------- IV (Nov 05 2001) ---------------------------- */
...
/* --------------------------- IV (Nov 05 2001) ---------------------------> */
    if (esr <= FL(0.0)) {				/* & chk consistency */
      printf("sr = %f, kr = %f, ksmps = %f\n", esr, ekr, ensmps);
      die("invalid sample rate");
    }
    if (ekr <= FL(0.0)) {
      printf("sr = %f, kr = %f, ksmps = %f\n", esr, ekr, ensmps);
      die("invalid control rate");
    }
    if ((ensmps <= FL(0.0)) || (ensmps != (MYFLT) ksmps)) {
      printf("sr = %f, kr = %f, ksmps = %f\n", esr, ekr, ensmps);
      die("invalid ksmps value");
    }
    if ((esr / ekr) != ensmps) {
      printf("sr = %f, kr = %f, ksmps = %f\n", esr, ekr, ensmps);
      die(Str(X_903,"inconsistent sr, kr, ksmps"));
    }
/* <-------------------------- IV (Nov 05 2001) ---------------------------- */
