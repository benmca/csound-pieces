...
{
    int ans = rand()%(max+1);
/*      printf("my_random(%d)=%d\n", max, ans); */
/*      if (ans<0 || ans>max) printf("my_random failure\n"); */
    return ans;
}

MYFLT float_random(MYFLT max) /* Return random float between 0.0 and max */
{
    MYFLT temp;
    temp = FL(32767.0) * (MYFLT) rand()/(MYFLT)RAND_MAX;
    temp = temp * FL(0.0000305185);
    temp = temp * max;
/*      printf("                     float_random(%f)=%f\n", max, temp); */
    return temp;
}

MYFLT noise_tick(void) /*  Return random MYFLT float between -1.0 and 1.0 */
{
    MYFLT temp =
...
void cabasaset(CABASA *p)
{
    ...
    p->gain = (MYFLT)log((double)CABA_NUM_BEADS) *
      CABA_GAIN / (MYFLT) CABA_NUM_BEADS;
    p->resons = CABA_RESON;
    p->coeffs1 = CABA_RESON * CABA_RESON;
    p->coeffs0 = - CABA_RESON * FL(2.0) *
      (MYFLT)cos((double)CABA_CENTER_FREQ * TWOPI / esr);
                                /* Note On */
    p->shakeEnergy = *p->amp * AMP_RSCALE * MAX_SHAKE * FL(0.1);
    if (p->shakeEnergy > MAX_SHAKE) p->shakeEnergy = MAX_SHAKE;
    p->last_num = FL(0.0);
}

void cabasa(CABASA *p)
{
    MYFLT *ar = p->ar;
    long nsmps = ksmps;
    MYFLT data;
...
/*        printf("damp/decay = %f/%f\n", *p->damp, p->systemDecay); */
    }

    if (*p->shake_max != FL(0.0)) {
      shakeEnergy = p->shakeEnergy += ksmps * *p->shake_max * MAX_SHAKE * FL(0.1);
...
void sekereset(SEKERE *p)
{
    ...
    p->gain = (MYFLT)log((double)SEKE_NUM_BEANS) *
      SEKE_GAIN / (MYFLT) SEKE_NUM_BEANS;
    p->resons = SEKE_RESON;
    p->coeffs1 = SEKE_RESON * SEKE_RESON;
    p->coeffs0 = - SEKE_RESON * FL(2.0) *
      (MYFLT)cos((double)SEKE_CENTER_FREQ * TWOPI / esr);
                                /* Note On */
    p->shakeEnergy = *p->amp * AMP_RSCALE * MAX_SHAKE * FL(0.1);
    if (p->shakeEnergy > MAX_SHAKE) p->shakeEnergy = MAX_SHAKE;
    p->last_num = FL(0.0);
...
void sandset(SEKERE *p)
{
    ...
    p->gain = (MYFLT)log((double)SANDPAPR_NUM_GRAINS) *
    ...
    p->coeffs0 = - SANDPAPR_RESON * FL(2.0) *
      (MYFLT)cos((double)SANDPAPR_CENTER_FREQ * TWOPI / esr);
                                /* Note On */
    p->shakeEnergy = *p->amp * AMP_RSCALE * MAX_SHAKE * FL(0.1);
    if (p->shakeEnergy > MAX_SHAKE) p->shakeEnergy = MAX_SHAKE;
    p->last_num = FL(0.0);
...
void stixset(SEKERE *p)
{
    ...
    p->gain = (MYFLT)log((double)STIX1_NUM_BEANS) *
      STIX1_GAIN / (MYFLT) STIX1_NUM_BEANS;
    p->resons = STIX1_RESON;
    p->coeffs1 = STIX1_RESON * STIX1_RESON;
    p->coeffs0 = - STIX1_RESON * FL(2.0) *
      (MYFLT)cos((double)STIX1_CENTER_FREQ * TWOPI / esr);
                                /* Note On */
    p->shakeEnergy = *p->amp * AMP_RSCALE * MAX_SHAKE * FL(0.1);
    if (p->shakeEnergy > MAX_SHAKE) p->shakeEnergy = MAX_SHAKE;
    p->last_num = FL(0.0);
...
void crunchset(CABASA *p)
{
    ...
    p->gain = (MYFLT)log((double)CRUNCH1_NUM_BEADS) *
    ...
    p->coeffs0 = - CRUNCH1_RESON * FL(2.0) *
      (MYFLT)cos((double)CRUNCH1_CENTER_FREQ * TWOPI / esr);
                                /* Note On */
    p->shakeEnergy = *p->amp * AMP_RSCALE * MAX_SHAKE * FL(0.1);
    if (p->shakeEnergy > MAX_SHAKE) p->shakeEnergy = MAX_SHAKE;
    p->last_num = FL(0.0);
...
void guiro(GUIRO *p)
{
    ...
    if (*p->num_teeth != FL(0.0) &&
      ...
      p->gains0 = p->gains1 = (MYFLT)log((double)p->num_objects) * GUIR_GAIN / (MYFLT) p->num_objects;
    }
...
/*        p->scrapeVel = p->shake_damp; */
    ...
    if (*p->freq != FL(0.0) && *p->freq !=  p->res_freqSave) {
      ...
      p->coeffs00 = -GUIR_GOURD_RESON * FL(2.0) * (MYFLT)cos((double)p->res_freqSave * TWOPI / esr);
    }
    if (*p->freq2 != p->res_freq2) {
      p->res_freq2 = *p->freq2;
      p->coeffs10 = -GUIR_GOURD_RESON2 * FL(2.0) * (MYFLT)cos((double)p->res_freq2 * TWOPI / esr);
    }
    if (p->kloop>0 && p->h.insdshead->relesing) p->kloop=1;
    if ((--p->kloop) == 0) {
...
void tambourine(TAMBOURINE *p)
{
    ...
    if (*p->freq != 0.0 && *p->freq != p->res_freq) {
      p->res_freq = *p->freq;
      p->coeffs00 = -TAMB_SHELL_RESON * FL(2.0) * (MYFLT)cos((double)p->res_freq * TWOPI / esr);
    }
    if (*p->damp != 0.0 && *p->damp != p->shake_damp) {
      p->shake_damp = *p->damp;
      p->systemDecay = TAMB_SYSTEM_DECAY + (p->shake_damp * FL(0.002));
    ...
    if (*p->freq1 != FL(0.0) && *p->freq1 != p->res_freq1) {
      p->res_freq1 = *p->freq1;
      p->coeffs10 = -TAMB_CYMB_RESON * FL(2.0) * (MYFLT)cos((double)p->res_freq1 * TWOPI / esr);
    }
    if (*p->freq2 != FL(0.0) && *p->freq2 != p->res_freq2) {
      p->res_freq2 = *p->freq2;
      p->coeffs20 = -TAMB_CYMB_RESON * FL(2.0) * (MYFLT)cos((double)p->res_freq2 * TWOPI / esr);
    }
    if (p->kloop>0 && p->h.insdshead->relesing) p->kloop=1;
    if ((--p->kloop) == 0) {
      p->shakeEnergy = FL(0.0);
...

    {
      ...
      do {
        ...
        if (my_random(1024) < p->num_objects) {
          ...
          p->coeffs10 = -TAMB_CYMB_RESON * FL(2.0) * (MYFLT)cos((double)temp_rand * TWOPI / esr);
          temp_rand = p->res_freq2 * (FL(1.0) + (FL(0.05) * noise_tick()));
          p->coeffs20 = -TAMB_CYMB_RESON * FL(2.0) * (MYFLT)cos((double)temp_rand * TWOPI / esr);
        }
...
void bambooset(BAMBOO *p)
{
    ...
    p->coeffs00 = -BAMB_RESON * FL(2.0) *
      (MYFLT)cos((double)BAMB_CENTER_FREQ0 * TWOPI / esr);
    p->coeffs11 = BAMB_RESON * BAMB_RESON;
    p->coeffs10 = -BAMB_RESON * FL(2.0) *
      (MYFLT)cos((double)BAMB_CENTER_FREQ1 * TWOPI / esr);
    p->coeffs21 = BAMB_RESON * BAMB_RESON;
    p->coeffs20 = -BAMB_RESON * FL(2.0) *
      (MYFLT)cos((double)BAMB_CENTER_FREQ2 * TWOPI / esr);
                                /* Note On */
    p->shakeEnergy = *p->amp * AMP_RSCALE * MAX_SHAKE * FL(0.1);
    p->shake_damp = FL(0.0);
    if (p->shakeEnergy > MAX_SHAKE) p->shakeEnergy = MAX_SHAKE;
...
void bamboo(BAMBOO *p)
{
    ...
    if (*p->freq != 0.0 && *p->freq != p->res_freq0) {
      p->res_freq0 = *p->freq;
      p->coeffs00 = -BAMB_RESON * FL(2.0) * (MYFLT)cos((double)p->res_freq0 * TWOPI / esr);
    }
    if (*p->damp != 0.0 && *p->damp != p->shake_damp) {
      p->shake_damp = *p->damp;
    ...
    if (*p->freq1 != 0.0 && *p->freq1 != p->res_freq1) {
      p->res_freq1 = *p->freq1;
      p->coeffs10 = -BAMB_RESON * FL(2.0) * (MYFLT)cos((double)p->res_freq1 * TWOPI / esr);
    }
    if (*p->freq2 != 0.0 && *p->freq2 != p->res_freq2) {
      p->res_freq2 = *p->freq2;
      p->coeffs20 = -BAMB_RESON * FL(2.0) * (MYFLT)cos((double)p->res_freq2 * TWOPI / esr);
    }
    if (p->kloop>0 && p->h.insdshead->relesing) p->kloop=1;
    if ((--p->kloop) == 0) {
      p->shakeEnergy = FL(0.0);
...

    {
      ...
      do {
        ...
        if (my_random(1024) < p->num_objects) {
          ...
          p->coeffs00 = -BAMB_RESON * FL(2.0) * (MYFLT)cos((double)temp_rand * TWOPI / esr);
          temp_rand = p->res_freq1 * (FL(1.0) + (FL(0.2) * noise_tick()));
          p->coeffs10 = -BAMB_RESON * FL(2.0) * (MYFLT)cos((double)temp_rand * TWOPI / esr);
          temp_rand = p->res_freq2 * (FL(1.0) + (FL(0.2) * noise_tick()));
          p->coeffs20 = -BAMB_RESON * FL(2.0) * (MYFLT)cos((double)temp_rand * TWOPI / esr);
        }
        inputs0 = sndLevel * noise_tick();      /* Actual Sound is Random */
        inputs1 = inputs0;
        inputs2 = inputs0;
...
void wuterset(WUTER *p)
{
    ...
    p->coeffs00 = -WUTR_RESON * FL(2.0) *
      (MYFLT)cos((double)WUTR_CENTER_FREQ0 * TWOPI / esr);
    p->coeffs11 = WUTR_RESON * WUTR_RESON;
    p->coeffs10 = -WUTR_RESON * FL(2.0) *
      (MYFLT)cos((double)WUTR_CENTER_FREQ1 * TWOPI / esr);
    p->coeffs21 = WUTR_RESON * WUTR_RESON;
    p->coeffs20 = -WUTR_RESON * FL(2.0) *
      (MYFLT)cos((double)WUTR_CENTER_FREQ2 * TWOPI / esr);
                                /* Note On */
    p->shakeEnergy = *p->amp * AMP_RSCALE * MAX_SHAKE * FL(0.1);
    p->shake_damp = FL(0.0);
    if (p->shakeEnergy > MAX_SHAKE) p->shakeEnergy = MAX_SHAKE;
...
void wuter(WUTER *p)
{
    ...
    if (*p->freq != 0.0 && *p->freq != p->res_freq0) {
      p->res_freq0 = *p->freq;
      p->coeffs00 = -WUTR_RESON * FL(2.0) * (MYFLT)cos((double)p->res_freq0 * TWOPI / esr);
    }
    if (*p->damp != 0.0 && *p->damp != p->shake_damp) {
      p->shake_damp = *p->damp;
    ...
    if (*p->freq1 != FL(0.0) && *p->freq1 != p->res_freq1) {
      p->res_freq1 = *p->freq1;
      p->coeffs10 = -WUTR_RESON * FL(2.0) * (MYFLT)cos((double)p->res_freq1 * TWOPI / esr);
    }
    if (*p->freq2 != 0.0 && *p->freq2 != p->res_freq2) {
      p->res_freq2 = *p->freq2;
      p->coeffs20 = -WUTR_RESON * FL(2.0) * (MYFLT)cos((double)p->res_freq2 * TWOPI / esr);
    }
    if (p->kloop>0 && p->h.insdshead->relesing) p->kloop=1;
    if ((--p->kloop) == 0) {
      p->shakeEnergy = FL(0.0);
...

    {
      ...
      do {
        ...
        if (my_random(32767) < num_objects) {
          ...
          j = my_random(3);
          if (j == 0)   {
            p->center_freqs0 = p->res_freq1 * (FL(0.75) + (FL(0.25) * noise_tick()));
            p->gains0 = (MYFLT)fabs(noise_tick());
          }
          else if (j == 1)      {
            p->center_freqs1 = p->res_freq1 * (FL(1.0) + (FL(0.25) * noise_tick()));
            p->gains1 = (MYFLT)fabs(noise_tick());
          }
          else  {
            p->center_freqs2 = p->res_freq1 * (FL(1.25) + (FL(0.25) * noise_tick()));
        ...
        if (p->gains0 >  FL(0.001)) {
          ...
          p->coeffs00 = -WUTR_RESON * FL(2.0) *
            (MYFLT)cos((double)p->center_freqs0 * TWOPI / esr);
        }
        p->gains1 *= WUTR_RESON;
        if (p->gains1 > FL(0.001)) {
          p->center_freqs1 *= WUTR_FREQ_SWEEP;
          p->coeffs10 = -WUTR_RESON * FL(2.0) *
            (MYFLT)cos((double)p->center_freqs1 * TWOPI / esr);
        }
        p->gains2 *= WUTR_RESON;
        if (p->gains2 > FL(0.001)) {
          p->center_freqs2 *= WUTR_FREQ_SWEEP;
          p->coeffs20 = -WUTR_RESON * FL(2.0) *
            (MYFLT)cos((double)p->center_freqs2 * TWOPI / esr);
        }
...
void sleighbells(SLEIGHBELLS *p)
{
    MYFLT *ar = p->ar;
    long nsmps = ksmps;
    MYFLT data;
    ...
    if (*p->num_bells != 0.0 && *p->num_bells != p->num_objects) {
      p->num_objects = *p->num_bells;
      if (p->num_objects < FL(1.0)) p->num_objects = FL(1.0);
    }
    if (*p->freq != 0.0 && *p->freq != p->res_freq0) {
      p->res_freq0 = *p->freq;
      p->coeffs00 = -SLEI_CYMB_RESON * FL(2.0) * (MYFLT)cos((double)p->res_freq0 * TWOPI / esr);
    }
    if (*p->damp != 0.0 && *p->damp != p->shake_damp) {
      p->shake_damp = *p->damp;
    ...
    if (*p->freq1 != 0.0 && *p->freq1 != p->res_freq1) {
      p->res_freq1 = *p->freq1;
      p->coeffs10 = -SLEI_CYMB_RESON * FL(2.0) * (MYFLT)cos((double)p->res_freq1 * TWOPI / esr);
    }
    if (*p->freq2 != 0.0 && *p->freq2 != p->res_freq2) {
      p->res_freq2 = *p->freq2;
      p->coeffs20 = -SLEI_CYMB_RESON * FL(2.0) * (MYFLT)cos((double)p->res_freq2 * TWOPI / esr);
    }
    if (p->kloop>0 && p->h.insdshead->relesing) p->kloop=1;
    if ((--p->kloop) == 0) {
      p->shakeEnergy = FL(0.0);
...

    {
      ...
      do {
        ...
        if (my_random(1024) < p->num_objects) {
          ...
          p->coeffs00 = -SLEI_CYMB_RESON * FL(2.0) * (MYFLT)cos((double)temp_rand * TWOPI / esr);
          temp_rand = p->res_freq1 * (FL(1.0) + (FL(0.03) * noise_tick()));
          p->coeffs10 = -SLEI_CYMB_RESON * FL(2.0) * (MYFLT)cos((double)temp_rand * TWOPI / esr);
          temp_rand = p->res_freq2 * (FL(1.0) + (FL(0.03) * noise_tick()));
          p->coeffs20 = -SLEI_CYMB_RESON * FL(2.0) * (MYFLT)cos((double)temp_rand * TWOPI / esr);
          temp_rand = p->res_freq3 * (FL(1.0) + (FL(0.03) * noise_tick()));
          p->coeffs30 = -SLEI_CYMB_RESON * FL(2.0) * (MYFLT)cos((double)temp_rand * TWOPI / esr);
          temp_rand = p->res_freq4 * (FL(1.0) + (FL(0.03) * noise_tick()));
          p->coeffs40 = -SLEI_CYMB_RESON * FL(2.0) * (MYFLT)cos((double)temp_rand * TWOPI / esr);
        }
        inputs0 = sndLevel * noise_tick();      /* Actual Sound is Random */
        inputs1 = inputs0;
        inputs2 = inputs0;
