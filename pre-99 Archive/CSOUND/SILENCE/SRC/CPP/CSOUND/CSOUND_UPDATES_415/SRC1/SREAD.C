...
#define MARGIN  2000            /* minimum remaining before new request */
...
static long expand_nxp(void)
{
    ...
    oldmem = curmem;
    n = (long) (memend - curmem);
    n += (1L + n / ((long) MEMSIZ << 3)) * (long) MEMSIZ;
    curmem = (char*) mrealloc ((void*) curmem, n * sizeof (char));
    memend = curmem + n;
    ofs = curmem - oldmem;	/* calculate offset */
    if (!(ofs)) return 0L;
    /* update all pointers */
    nxp += ofs;
    if (sp != NULL) sp += ofs;
    if (frstbp != NULL) frstbp = (SRTBLK*) ((char*) frstbp + ofs);
    if (bp != NULL) bp = (SRTBLK*) ((char*) bp + ofs);
    if (prvibp != NULL) prvibp = (SRTBLK*) ((char*) prvibp + ofs);
    tmpblk = frstbp;
    while (tmpblk != NULL) {
      if (tmpblk->nxtblk != NULL)
        tmpblk->nxtblk = (SRTBLK*) ((char*) tmpblk->nxtblk + ofs);
      if (tmpblk->prvblk != NULL)
        tmpblk->prvblk = (SRTBLK*) ((char*) tmpblk->prvblk + ofs);
      tmpblk = tmpblk->nxtblk;
    }
    /* return offset by which score text space was moved */
    return ofs;
 margerr:
...
{                             /*  alloc 1st memblk if nec; init *nxp to this */
    if (curmem == NULL) {
      curmem = (char*) mmalloc ((long) MEMSIZ * sizeof (char));
      memend = curmem + (long) MEMSIZ;
    }
    nxp = (char*) curmem;
}
/* <-------------------------- IV (Nov 05 2001) ---------------------------- */
...
{                               /*    called at completion of sort */
    if (curmem != NULL) mfree (curmem);
    curmem = memend = nxp = NULL;
}
/* <-------------------------- IV (Nov 05 2001) ---------------------------- */
...
/* --------------------------- IV (Nov 05 2001) ---------------------------> */
        /* **** CHECK **** */
        if (memend - p < MARGIN) {     /* if this memblk exhausted lengthen*/
          p += expand_nxp();
        }
        /* **** END CHECK **** */
/* <-------------------------- IV (Nov 05 2001) ---------------------------- */
