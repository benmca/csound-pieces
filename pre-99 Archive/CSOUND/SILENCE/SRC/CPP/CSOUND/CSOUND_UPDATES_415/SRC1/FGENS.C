...
static void   gen23(void), gen24(void), gen16(void);
/* --------------------------- IV (Nov 05 2001) ---------------------------> */
...
{				/* stops reading when table is full	*/
    if (nargs < 4)
      FTERR(Str(X_938,"insufficient args"))
    if (O.gen01defer) {
...
gn17err: fterror(Str(X_794,"gen call has illegal x-ordinate values:"));
}
...
void ftgen(FTGEN *p)                    /* set up and call any GEN routine */
{
    ...
    *fp++ = *p->p5;
    ...
    ftevt->p[2] = FL(0.0);      		  /* force time 0    */
    if (ftevt->p[5] == SSTRCOD) {                 /* if string p5    */
      if (ftevt->p[4] == FL(1.0)) {               /*   must be Gen01 */
