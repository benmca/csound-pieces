...
void dspset(DSPLAY *p)
{
    ...
    p->nxtp = (MYFLT *) auxp;
    p->pntcnt = npts;
    sprintf(strmsg,Str(X_932,"instr %d, signal %s:"),
            p->h.insdshead->insno, p->STRARG);
    dispset(&p->dwindow,(MYFLT *)auxp,bufpts,strmsg,
            (int)*p->iwtflg,Str(X_703,"display"));
}
...
void kdsplay(DSPLAY *p)
{
    ...
    if (p->auxch.auxp==NULL) { /* RWD fix */
      initerror(Str(X_704,"display: not initialised"));
      return;
    }
    if (!p->nprds) {
      *fp++ = *p->signal;
...
void kdspfft(DSPFFT *p)
{
    ...
    if (p->auxch.auxp==NULL) { /* RWD fix */
      initerror(Str(X_702,"dispfft: not initialised"));
      return;
    }
...
void tempeset(TEMPEST *p)
{
    ...
    if ((p->dtimcnt = (int)(ekr * *p->idisprd)) < 0)
      initerror(Str(X_851,"illegal idisprd"));
    if ((p->tweek = *p->itweek) <= 0)
      initerror(Str(X_865,"illegal itweek"));
    if (iperiod != FL(0.0)) {
      if ((minlam = (int)(*p->imindur/iperiod)) <= 0)
        initerror(Str(X_855,"illegal imindur"));
      if ((npts = (int)(*p->imemdur / iperiod)) <= 0)
        initerror(Str(X_854,"illegal imemdur"));
    }
    if (*p->ihtim <= FL(0.0))
      initerror(Str(X_853,"illegal ihtim"));
    if (*p->istartempo <= FL(0.0))
      initerror(Str(X_884,"illegal startempo"));
    ftp = ftfind(p->ifn);
    if (ftp != NULL && *ftp->ftable == 0.)
      initerror(Str(X_835,"ifn table begins with zero"));
    if (inerrcnt)
...
void tempest(TEMPEST *p)
{
    ...
    if (p->auxch.auxp==NULL) { /* RWD fix */
      initerror(Str(X_1276,"tempest: not initialised"));
      return;
    }
