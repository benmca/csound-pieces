                                ...
                                /* Return -1 on failure */
{
    ...
    if ((hdr=readheader(sinfd,sfname,p)) != NULL /* if headerblk returned */
      ...
      if (p->format && hdr->format != p->format) {
        sprintf(errmsg,Str(X_1204,
                           "soundin %s superceded by %s header format %s"),
      ...
      if (p->analonly) {
        if (!p->sr) {
          p->sr = (long)DFLT_SR;
          sprintf(errmsg,
                  Str(X_1016,"no -s and no soundheader, using sr default %ld"),
                  p->sr);
          warning(errmsg);
        }
      }
      ...
      sprintf(errmsg,
              Str(X_55,"%s has no soundfile header, "
    ...
    if (p->nchnls == 1)
      ...
      printf(Str(X_64,"%s, reading "),
             p->nchnls == 2 ? Str(X_1246,"stereo") :
             p->nchnls == 4 ? Str(X_1148,"quad") :
             p->nchnls == 6 ? Str(X_830,"hex") :
             p->nchnls == 8 ? Str(X_1088,"oct") :
             Str(X_1556,"chanels-"), p->nchnls);
                             ...
                             /* shares above sndgetset with SAsndgetset, gen01*/
{
    ...
    if (p->fdch.fd != 0) {  /* if file already open, rtn */
      ...
      lseek(p->fdch.fd,(off_t)(p->datpos+(skipframes* p->sampframsiz)),SEEK_SET);
      return;
    }
      ...
      samp_24:
        ...
        case 1:
          do {
            ...
            s24.bytes[2] = *inbufp++;
            s24.bytes[3] = *inbufp++;
            val = (float)((double)(s24.lsamp) * INLONGFAC );
            *r1++ = (MYFLT)val;
          } while (--nsmps);
          break;
        case 2:
          do {
            ...
            s24.bytes[2] = *inbufp++;
            s24.bytes[3] = *inbufp++;
            *r1++ = (MYFLT)((double)(s24.lsamp) * INLONGFAC );
            s24.bytes[1] = *inbufp++;
            s24.bytes[2] = *inbufp++;
            s24.bytes[3] = *inbufp++;
            *r2++ = (MYFLT)((double)(s24.lsamp) * INLONGFAC );
          } while (--nsmps);
          break;
        case 4:
          do {
            ...
            *r2++ = (MYFLT)((double)(s24.lsamp) * INLONGFAC );
            ...
            s24.bytes[2] = *inbufp++;
            s24.bytes[3] = *inbufp++;
            *r3++ = (MYFLT)((double)(s24.lsamp) * INLONGFAC );
            s24.bytes[1] = *inbufp++;
            s24.bytes[2] = *inbufp++;
            s24.bytes[3] = *inbufp++;
            *r4++ = (MYFLT)((double)(s24.lsamp) * INLONGFAC );
          } while (--nsmps);
        }
        if (inbufp >= p->bufend) {
          if ((n = sreadin(p->fdch.fd,p->inbuf,SNDIN24BUFSIZ,p)) == 0) {
            p->endfile = 1;
            if (ntogo)
              goto filend;
            else return;
