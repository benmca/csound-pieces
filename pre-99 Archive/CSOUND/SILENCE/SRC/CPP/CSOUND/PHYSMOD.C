...
void clarinset(CLARIN *p)
{
    ...
    if ((ftp = ftfind(p->ifn)) != NULL) p->vibr = ftp;
      ...
      perferror(Str(X_376,"No table for Clarinet")); /* Expect sine wave */
      return;
    }
...
void fluteset(FLUTE *f)
{
    ...
    if ((ftp = ftfind(f->ifn)) != NULL) f->vibr = ftp;
      ...
      perferror(Str(X_378,"No table for Flute")); /* Expect sine wave */
      return;
    }
...
{                                              /*  sample is differential  */
    ...
    b->lastOutput = intpow(b->lastOutput,-4L);
...
void bowedset(BOWED *b)
{
    ...
    if ((ftp = ftfind(b->ifn)) != NULL) b->vibr = ftp;
      ...
      perferror(Str(X_386,"No table for wgbow vibrato")); /* Expect sine wave */
      return;
    }
...
int DLineA_setDelay(DLineA *p, MYFLT lag)
{
    ...
    if (p->length<=0) {
      perferror(Str(X_247,"DlineA not initialised"));
      return 1;
    }
...
void brassset(BRASS *p)
{
    ...
    if ((ftp = ftfind(p->ifn)) != NULL) p->vibr = ftp;
      ...
      perferror(Str(X_375,"No table for Brass")); /* Expect sine wave */
      return;
    }
...
void brass(BRASS *p)
{
    ...
    if (p->frq != *p->frequency) {             /* Set frequency if changed */
      ...
      if (DLineA_setDelay(&p->delayLine, p->slideTarget)) return; 
      p->lipTarget = p->frq;
