...
void foscil(FOSC *p)
{
    ...
    ftp = p->ftp;
    if (ftp==NULL) {
      initerror(Str(X_774,"foscil: not initialised"));
      return;
    }
    ftab = ftp->ftable;
    lobits = ftp->lobits;
...
void foscili(FOSC       *p)
{
    ...
    ftp = p->ftp;
    if (ftp==NULL) {        /* RWD fix */
      initerror(Str(X_775,"foscili: not initialised"));
      return;
    }
    lobits = ftp->lobits;
    mphs = p->mphs;
...
void adsyn(ADSYN *p)
{
    ...
    if (isintab==NULL) {    /* RWD fix */
      initerror(Str(X_585,"adsyn: not initialised"));
      return;
    }
    ...
    nsmps = ksmps;
    do
      *ar++ = *sp++ * ampscale;           /* float & scale the results */
    while (--nsmps);
}
