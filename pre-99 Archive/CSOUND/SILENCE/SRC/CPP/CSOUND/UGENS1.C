...
/*              counter, p->curcnt, counter*ksmps, (double)(counter*ksmps)/ekr, */
/*              p->curval, p->curinc); */
    }
/*     counter++; */
}
...
void linseg(LINSEG *p)
{
    ...
    if (p->auxch.auxp==NULL) {  /* RWD fix */
      initerror(Str(X_967,"linseg: not initialised (arate)\n"));
    }
...
void kxpseg(EXXPSEG *p)
{
    ...
    segp = p->cursegp;
    if (p->auxch.auxp==NULL) { /* RWD fix */
      initerror(Str(X_750,"expseg (krate): not initialised"));
      return;
    }
    while (--segp->cnt < 0)
      p->cursegp = ++segp;
...
void expseg(EXXPSEG *p)
{
    ...
    segp = p->cursegp;
    if (p->auxch.auxp==NULL) { /* RWD fix */
      initerror(Str(X_749,"expseg (arate): not initialised"));
      return;
    }
    while (--segp->cnt < 0)
      p->cursegp = ++segp;
...
void knvlpx(ENVLPX *p)
{
    ...
    ftp = p->ftp;
    if (ftp==NULL) {        /* RWD fix */
      initerror(Str(X_720,"envlpx(krate): not initialised"));
      return;
    }
