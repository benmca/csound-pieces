...
/* static MYFLT   invkr; */
extern MYFLT onedkr; /*  contains 1/kr   created and initialised in oload.c */
...
{
    short *xtra = &(p->h.insdshead->xtratim);
    short tim = (short) (*p->extradur * ekr);
...
void kvar_out_on_set1(KOUT_ON *p)
{
  printf(Str(X_429,"Please use midion rather than kon\n"));
  kvar_out_on_set(p);
}
