...
void vbap_SIXTEEN_moving_control(VBAP_SIXTEEN_MOVING  *p);

void vbap_SIXTEEN(VBAP_SIXTEEN  *p) /* during note performance:   */
{
    MYFLT *outptr, *inptr;
    ...
    for (j=0; j<SIXTEEN; j++) {
      ...
      if (ngain != FL(0.0) || ogain != FL(0.0))
        if (ngain != ogain) {
          for (i = p->counter; i < ksmps + p->counter; i++) {
            *outptr++ = *inptr++ *
...
void vbap_SIXTEEN_control(VBAP_SIXTEEN  *p)
{
...
void vbap_SIXTEEN_init(VBAP_SIXTEEN  *p)
{                               /* Initializations before run time*/
    ...
    p->out_array[9] = p->result9;  /* outputs */
    p->out_array[10] = p->result10;
    p->out_array[11] = p->result11;
    p->out_array[12] = p->result12;
    p->out_array[13] = p->result13;
    p->out_array[14] = p->result14;
    p->out_array[15] = p->result15;
    p->out_array[16] = p->result16;
...
void vbap_SIXTEEN_moving(VBAP_SIXTEEN_MOVING  *p) /* during note performance: */
{
    MYFLT *outptr, *inptr;
    MYFLT ogain, ngain, gainsubstr;
    MYFLT invfloatn;
    ...
    /* write audio to resulting audio streams wSIXTEENed
       with gain factors*/
    invfloatn =  FL(1.0) / (MYFLT) ksmps;
    for (j=0; j<SIXTEEN ;j++) {
      inptr = p->audio;
...
void vbap_SIXTEEN_moving_control(VBAP_SIXTEEN_MOVING  *p)
{
    ...
    if (*p->field_am >= FL(0.0)) { /* point-to-point */
      if (p->dim == 3) { /* 3-D*/
        ...
        tmp3.x = (1-coeff) * tmp1.x + coeff * tmp2.x;
        tmp3.y = (1-coeff) * tmp1.y + coeff * tmp2.y;
        tmp3.z = (1-coeff) * tmp1.z + coeff * tmp2.z;
...
void vbap_SIXTEEN_moving_init(VBAP_SIXTEEN_MOVING  *p)
{
    int i,j;
    MYFLT *ptr;
    LS_SET *ls_set_ptr;
