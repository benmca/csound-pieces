    ...
    char *endchar)
{
  ...
  printf(" measure length=%f\n",ratval(timesig));
	...
	*transpose=0;
	...
	nextpp->dst=z;
      ...
      else scoterror(Str(X_515,"Unrecognized keyword"));
      break;
    case '{':
      findint(&c);
      expectchar(':');
    ...
    case '<':
      if (pn==NULL)
      {
	scoterror(Str(X_475,"Syntax error: can't back up"));
	break;
      }
      if (pn->next==NULL)
      {
  ...
  printf(" slur=%d tie=%d\n",nn->slur,nn->tie);
	...
	pn->tie=nn->tie;
        freenote(nn);
	nn=pn;
	if (c==(char) '[') scoterror(Str(X_534,"Warning: params changed on tie"));
      }
      else
      {
  ...
  printf("   Yes.\n");
	...
	writenote(pn);
      ...
      if (nn!=pn)
      {
	if (!pn->written) scoterror(Str(X_330,"Lost previous note: not written"));
...
Inst **insttopp;
{
  ...
  for (;;)
  {
    ...
    if (findint((int *)&p->number)) scoterror(Str(X_477,"Syntax error: no number"));
