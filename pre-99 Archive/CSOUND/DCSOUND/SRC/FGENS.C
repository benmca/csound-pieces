...
static void   gen22(void), gen22raw(void), gen23(void),	gen24(void), gen16(void); /* gab-A1 */
static void   gen25(void), gen27(void), gen28(void),gen40(void),gen41(void),gen42(void);
static void   gen43(void);

static void   GENUL(void);

/*gab-A1 (for gogins library)*/
/*static*/	FUNC	**flist /*[MAXFNUM+1]*/;
static FUNC *ftp;
//static  int	fmax = 0;
static  int	maxfnum = 0;
static	GEN	gensub[GENMAX+1] = { GENUL, gen01, gen02, gen03, gen04, gen05,
...
void fgens(EVTBLK *evtblkp)	/* create ftable using evtblk data */
{
	...
	if (fno > maxfnum) {
		...
		size++;
		if (maxfnum!=0) printf("Increasing number of tables from %d to %d\n", maxfnum, size-1);
		nn = (FUNC**)mrealloc(flist, size*sizeof(FUNC*));
		flist = nn;
		for (i=maxfnum; i<size; i++) flist[i] = NULL; /* Clear new section */
		maxfnum = size-1;
		/*           FTERR("illegal ftable number") */
	}
	...
	if ((flen = (long)e->p[3])) {			/* if user flen	given       */
		...
		lomod = MAXLEN / flen;
		ftp->lomask = lomod - 1;
		ftp->lodiv = 1.0f/((MYFLT)lomod);	/*    & other useful vals    */
		tpdlen = TWOPI_F / flen;
		ftp->nchnls = 1;                    /*    presume mono for now   */
		ftp->flenfrms = flen;
		
	}
	else if (genum != 1 && genum != 28 && genum != 22 && genum != 23) /* else defer alloc to gen01|gen28 */ /* gab-A1 (gen22) */
		FTERR("deferred size for GEN01,GEN22,GEN23 and GEN28 only")
		else ftp = NULL; 
	printf("ftable %d:\n", fno);
	...
	for (j = 0; j < 3; j++) {
		if (genum == 22 || genum == -22) /* gab-A1 */
			;//((short *) ftp->ftable)[flen + j] = ((short *)ftp->ftable)[j];
		else
			ftp->ftable[flen+j] = ftp->ftable[j]; /* gab d6 - put three guard points at the end of each table for cubic interpolation */
	}
}
...
extern  long    getsndin(int, MYFLT *, long, SOUNDIN *);
    ...
    ftp->gen01args.sample_rate = curr_func_sr; /* gab-A1 */
...
static void gen11(void)
{
	...
	if (nargs == 1 || k == 1 && r == 1.) {     /* simple "buzz" case */
		...
		tnp1 = (n << 1) + 1;
		scale = 0.5f / n;
		pdlen = (MYFLT)tpdlen / 2.0f;
		for (phs = 0; fp <= finp; phs++) {
			x = phs * pdlen;
			if (!(denom = (MYFLT)sin(x)))
				*fp++ = 1.0f;
			else *fp++ = ((MYFLT)sin(tnp1 * x) / denom - 1.0f) * scale;
		}
	}
		...
		twor  = r * 2.0f;
		rsqp1 = r * r + 1.0f;
		rtn   = (MYFLT)pow((double) r, (double) n);
		rtnp1 = rtn * r;
		if ((absr = (MYFLT)fabs(r)) > .999f && absr < 1.001f)
			scale = 1.0f / n;
		else scale = (1.0f - absr) / (1.0f - (MYFLT)fabs(rtn));
		for (phs=0; fp <= finp; phs++) {
			x = phs * tpdlen;
			numer = (MYFLT)cos(x*k) - r * (MYFLT)cos(x*km1) - rtn * (MYFLT)cos(x*kpn)
				+ rtnp1 * (MYFLT)cos(x*kpnm1);
			if ((denom = rsqp1 - twor*(MYFLT)cos(x)) > .0001f
			  || denom < -.0001f)
...
gen28err1: fterror(Str(X_672,"couldn't open space file")); return;
...
{			/*	set ftp to point to that structure	*/
	...
	if ((ftp = flist[fno]) == NULL) {	/*   alloc space as reqd */
		ftp = (FUNC *) mcalloc((long)sizeof(FUNC) + (flen + 3)*sizeof(MYFLT));	/*gab d6*/
		flist[fno] = ftp;
	}
}
...
/* gab-A1 */
FUNC *
ftfind2(MYFLT *argp)  /* added for allowing ftlen2() can return table size */
{					/* see ftlen() in AOPS.C */
register int	fno;
register FUNC	*ftp;
...
{			                 /*	set ftp to point to that structure	*/
	if ((ftp = flist[fno]) != NULL) {
	    ...
	    if (flen != ftp->flen) {    	/* if redraw & diff len, */
	        ...
	        short	*fp = (short *) ftp->ftable; //gab
			short	*finp = (short *) &ftp->ftable[flen/((sizeof(MYFLT) == 2) ? 2:4)]; //gab
			while (fp <= finp)
		    *fp++ = 0;
	    }
	}
	if ((ftp = flist[fno]) == NULL) {	/*   alloc space as reqd */
	    ftp = (FUNC *) mcalloc((long)sizeof(FUNC) + (flen+3)*sizeof(short)); //GAB!!!
	    flist[fno] = ftp;
	}
}
...
{
	MYFLT	*fp = ftp->ftable, *fp_source;  // array valori destinazione
	FUNC	*srcftp;	 // srcftp -> array valori sorgente
	int	srcno, srcpts, j;
...
FUNC *hfgens(EVTBLK *evtblkp)               /* create ftable using evtblk data */
{
	...
	//  FTPLERR("deferred size for GEN01 only")
	else if (genum != 1 && genum != 28 && genum != 22 && genum != 23) /* else defer alloc to gen01|gen28 */ /* gab-A1 (gen22) */
		FTPLERR("deferred size for GEN01,GEN22,GEN23 and GEN28 only")
