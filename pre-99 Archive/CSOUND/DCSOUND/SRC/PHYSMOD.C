...
MYFLT ReedTabl_LookUp(ReedTabl *r, MYFLT deltaP)
    /*   Perform "Table Lookup" by direct clipped  */
    /*   linear function calculation               */
{   /*   deltaP is differential reed pressure      */
    r->lastOutput = r->offSet + (r->slope * deltaP); /* basic non-lin */
    if (r->lastOutput > FL(1.0))
      r->lastOutput = FL(1.0);      /* if other way, reed slams shut */
    if (r->lastOutput < -FL(1.0))
      r->lastOutput = -FL(1.0);     /* if all the way open, acts like open end */
/*     printf("ReedT: Lookup=%f\n", r->lastOutput); */
    return r->lastOutput;
}
...
{
    printf("OneZero: gain=%f inputs=%f zeroCoeff=%f sgain=%f last=%f\n",
           p->gain, p->inputs, p->zeroCoeff, p->sgain, p->lastOutput);
}

/* *********************************************************************** */
void clarinset(CLARIN *p)
{
    FUNC	*ftp;
    MYFLT amp = (*p->amp)*AMP_RSCALE; /* Normalise */

    if ((ftp = ftfind(p->ifn)) != NULL)	p->vibr = ftp;
    else perferror(Str(X_376,"No table for Clarinet")); /* Expect sine wave */
...
/*            p->h.insdshead->offtim, p->kloop); */
      p->envelope.rate =  amp /(*p->attack*esr); /* Over 0.1sec */
      p->envelope.target = FL(0.55) + amp*FL(0.30); /* ?? values here */
    }
}

void clarin(CLARIN *p)
{
    MYFLT *ar = p->ar;
    long nsmps = ksmps;
    MYFLT amp = (*p->amp)*AMP_RSCALE; /* Normalise */
...
MYFLT JetTabl_lookup(MYFLT sample) /* Perform "Table Lookup"  */
...
void fluteset(FLUTE *f)
{
    FUNC	*ftp;
    long	length;

    if ((ftp = ftfind(f->ifn)) != NULL)	f->vibr = ftp;
    else perferror(Str(X_378,"No table for Flute")); /* Expect sine wave */
...
{                                              /*  sample is differential  */
    ...
    b->lastOutput = (MYFLT)fabs(input) + FL(0.75); /*  below min delta, frict = 1 */
    b->lastOutput = (MYFLT)pow((double)b->lastOutput,-4.0);
/* if (b->lastOutput < FL(0.0) ) b->lastOutput = FL(0.0); */ /* minimum frict is 0.0 */
    if (b->lastOutput > FL(1.0)) b->lastOutput = FL(1.0); /*  maximum friction is 1.0 */
    return b->lastOutput;
}
...
void bowedset(BOWED *b)
{
    ...
    else perferror(Str(X_386,"No table for wgbow vibrato")); /* Expect sine wave */
...
void DLineA_setDelay(DLineA *p, MYFLT lag)
{
    MYFLT outputPointer;
    outputPointer = (MYFLT)p->inPoint - lag + FL(2.0);  /* outPoint chases inpoint        */
				                    /*   + 2 for interp and other     */
    if (p->length<=0) perferror(Str(X_247,"DlineA not initialised"));
...
void LipFilt_setFreq(LipFilt *p, MYFLT frequency)
{
    ...
    coeffs[0] = FL(2.0) * FL(0.997) *
      (MYFLT)cos(TWOPI * (double)(frequency / esr)); /* damping should  */
...
void brassset(BRASS *p)
{
    FUNC	*ftp;
    MYFLT amp = (*p->amp)*AMP_RSCALE; /* Normalise */

    if ((ftp = ftfind(p->ifn)) != NULL)	p->vibr = ftp;
    else perferror(Str(X_375,"No table for Brass")); /* Expect sine wave */
