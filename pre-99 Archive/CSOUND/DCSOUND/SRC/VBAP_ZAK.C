...
void vbap_zak_moving_control(VBAP_ZAK_MOVING *);
void vbap_zak_control(VBAP_ZAK *);

void vbap_zak(VBAP_ZAK *p) /* during note performance:   */
{
    MYFLT *outptr, *inptr;
...
void vbap_zak_control(VBAP_ZAK *p)
{
...
/*      gains = (MYFLT)malloc(n*sizeof(MYFLT)); */
    ...
    p->out_array = zastart + (indx * ksmps);/* outputs */
    auxalloc(p->n*sizeof(MYFLT)*4, &p->auxch);
    p->curr_gains = (MYFLT*)p->auxch.auxp;
    p->beg_gains = p->curr_gains + p->n*sizeof(MYFLT);
    p->end_gains = p->beg_gains + p->n*sizeof(MYFLT);
    p->updated_gains = p->end_gains + p->n*sizeof(MYFLT);
...
void vbap_zak_moving(VBAP_ZAK_MOVING *p) /* during note performance:   */
{
    MYFLT *outptr, *inptr;
    MYFLT ogain, ngain, gainsubstr;
    MYFLT invfloatn;
...
void vbap_zak_moving_control(VBAP_ZAK_MOVING  *p)
{
    CART_VEC spreaddir[16];
    CART_VEC spreadbase[16];
    ANG_VEC atmp;
...
void vbap_zak_moving_init(VBAP_ZAK_MOVING  *p)
{
    int i,j, indx;
    MYFLT *ptr;
    LS_SET *ls_set_ptr;
