...
int pvlook(int argc, char *argv[])
{
...
#ifdef CWIN
    {
      char *name;
      char *oname;
      int __cdecl pvlook_dialog(int *,int *,int *,int *,int *, char **, char **);
      if (pvlook_dialog(&firstBin, &lastBin, &firstFrame, &lastFrame,
			&printInts, &name, &oname)==0)
        return 0;
      if ( ( fp = fopen(name, "r" ) ) == NULL ) {
        fprintf( stderr, "pvrlook: Unable to fdopen '%s'\n", name ) ;
        exit( -1 ) ;
      }
      ...
      if (lastBin== -1) lastBin = (framesize/2);
      if (lastFrame== -1) lastFrame = numframes;
      
      if (oname != NULL) outfd = fopen(oname, "w");
      else outfd = stdout;
    }
#else
    if ( argc == 1 ) {
    ...
    fprintf(outfd,"; Frames in Analysis: %d\n", ((phdr->dataBsize/4) / framesize));
