...
void tranRESET(void)
{
	...
	if (gbl.names != NULL) {
		   mfree(gbl.names); /* gab d4*/
		   gbl.names = NULL;
		   //mfree(gbl.namlim); // = gbl.names + GNAMES;
		   //mfree(gbl.nxtslot); // = gbl.names;
		   mfree(gbl.next); /* gab d4*/  // = NULL;
		   mfree(lcl.names); /* gab d4*/ // = (NAME *)mmalloc((long)(LNAMES*sizeof(NAME)));
		   //mfree(lcl.namlim); // = lcl.names + LNAMES;
		   mfree(lcl.next); /* gab d4*/ // = NULL;
		   //mfree(instrtxtp); /* gab d4*/ // = (INSTRTXT **)mcalloc((long)((1+maxinsno)*sizeof(INSTRTXT*)));
		   mfree(argoffspace); /* gab d4*/ // = (short *) mmalloc((long)argoffsize);   /* as argoff shorts */
		   mfree(pool); // = (float *)mmalloc((long)NCONSTS * sizeof(float));
		   
		   mfree(opcodlst ); /* gab d4*/
	}
	opcodlst = NULL;
}
#endif
...
void otran(void)
{
	...
	case ENDIN:
	    ...
	    VMSG( { putop(&bp->t);
	            printf("pmax %ld, kcnt %d, dcnt %d, wcnt %d, acnt %d\n",
...
#ifdef	VST /*gab e1*/
	{
	#define VST_BLOCK_RATIO 8
	#include "soundio.h"
	extern long VSTblock_size;
	extern float ensmps;
	extern int getsizformat(int);
	extern int VSTblockRatio, CheckVstBuffer, ndxVstBuffer;
	extern float *spoutVST,*spinVST;
	VSTblockRatio = VST_BLOCK_RATIO;
	CheckVstBuffer = 1;
	ndxVstBuffer = 0;
	ensmps = tran_ksmps = (float) VSTblock_size / VSTblockRatio;
	O.outbufsamps = VSTblock_size*2;
	spoutVST = (float *) mcalloc(O.outbufsamps * sizeof(float));
	spinVST = (float *) mcalloc(O.outbufsamps * sizeof(float));
	ksmps = (int) tran_ksmps;
	tran_kr = -1.0f;
	O.informat = O.outformat = AE_VST;
	O.outsampsiz = O.insampsiz = getsizformat(O.outformat);
	...
	if (tran_ksmps == -FL(1.0)) {
      ...
      tran_ksmps = (MYFLT)((int) (tran_sr / tran_kr));
		...
		ksmps = (int)tran_ksmps; /*gab e1*/ /* Ensure integer */
		if (tran_sr == -1) tran_sr = DFLT_SR;
		ekr = tran_kr = tran_sr / tran_ksmps; /*gab e1*/
    }
    if (tran_sr == -FL(1.0)) tran_sr = DFLT_SR;
...
static short plgndx(char *s)	/* get storage ndx of const, pnum, lcl or gbl */
				...
				/* poolcount & lclpmax are finalized */
{
    char	c;
    short	n, indx;
...
/* static void txtcpy(char *s, char *t) */
/* { */
/*     int n = sizeof(TEXT); */
/*     while (n--) */
/* 	*s++ = *t++; */
