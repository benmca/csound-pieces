...
void filterSet(filter* filt, MYFLT *c)
{
  ...
  if (!filt->buffer.inited)
    error(Str(X_277,"Filter not inited, can't set"),"<filter::set>");
...
/*    int i; */
/*    for (i=0;i<filt->buffer.size;i++) */
/*      filt->buffer.data[i]=FL(0.0); */
/*  } */

/* ::get -- check range of index, return corresponding coefficient */
/*  MYFLT filterGet(filter* filt, len_t i) */
/*  { */
/*    if ((i<0) || (i>(filt->buffer.size-1))) */
/*      error(Str(X_308,"Index out of range"),"<filter::get()>"); */
...
/*   MYFLT filterIIR(filter* filt, MYFLT s) */
/*  { */
/*    MYFLT *c=filt->coeffs; */
/*    int i; */
