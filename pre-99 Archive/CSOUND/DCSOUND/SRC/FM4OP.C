...
void make_FM4Op(FM4OP *p)
{
    ...
    else perferror(Str(X_384,"No table for VibWaveato")); /* Expect sine wave */
    p->baseFreq = FL(440.0);
    p->ratios[0] = FL(1.0);
    p->ratios[1] = FL(1.0);
    p->ratios[2] = FL(1.0);
...
void FM4Op_loadWaves(FM4OP *p)
{
    ...
    else perferror(Str(X_377,"No table for FM4Op")); /* Expect sine wave */
    if ((ftp = ftfind(p->ifn1)) != NULL) p->waves[1] = ftp;
    else perferror(Str(X_373,"No table for  FM4Op")); /* Expect sine wave */
...
{
    make_FM4Op(p);
    FM4Op_loadWaves(p);         /* Mixed -- 2 x sine; 1 x fwavblnk */
...
void percfluteset(FM4OP *p)
{
    ...
    make_FM4Op(p);
    FM4Op_loadWaves(p);         /* 3 x sines; 1 x fwavblnk */
