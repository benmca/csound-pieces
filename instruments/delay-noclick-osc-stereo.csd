...
aregenerated_signal1 init 0	; regenerated signal - added to delay output * regen setting 
aregenerated_signal2 init 0	; regenerated signal - added to delay output * regen setting 
...
ainputsig1 		inch kchan
ainputsig2 		inch kchan+1
ainputsig1 = ainputsig1 * kinput_volume
ainputsig2 = ainputsig2 * kinput_volume

noread:
asig_for_delayline1 = (ainputsig1 + aregenerated_signal1) * kregeneration_scalar
asig_for_delayline2 = (ainputsig2 + aregenerated_signal2) * kregeneration_scalar
kactive = 0
kactive_time times
...
aout_total1  delayr     gidelsize
aoutnew1   	deltapi     kcrossfade_after
aoutold1  	deltapi     kcrossfade_before
			delayw      asig_for_delayline1

aout_total2  delayr     gidelsize
aoutnew2   	deltapi     kcrossfade_after
aoutold2  	deltapi     kcrossfade_before
			delayw      asig_for_delayline2

aout1 = ainputsig1 + (aoutnew1 * kcf) + (aoutold1 * (1.0-kcf))
aout2 = ainputsig2 + (aoutnew2 * kcf) + (aoutold2 * (1.0-kcf))
aregenerated_signal1 = aout1
aregenerated_signal2 = aout2

readquery:
if 	koutput_on_off == 0	kgoto off
read:
outs	aout1*koutput_volume, aout2*koutput_volume
aout1 = aout1
aout2 = aout2
kgoto out
off:
