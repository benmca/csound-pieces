...
giosc_handle OSCinit 8000
	...
	opcode GlitchDelay, a, iiiiiSSSSSSSS
idelsize, imin_tap_point, icrossfadetime, ioschandle, icrossfade_instrument, SControllerIP, 
...
if  ((kcrossfade_before != kdelay_tap_point) || kactive != 0) then
	printks "checking....", .01
	if (kcrossfade_in_progress == 1 && kactive == 0) then
		printks "event is ended\n", .01
        kcrossfade_in_progress = 0
        kcrossfade_before = kcrossfade_after
    elseif (kcrossfade_in_progress == 1 && kactive == 1) then
...
if (kcrossfade_in_progress == 1) then
	kactive = 1
	
	afadein   linseg    0.0, icrossfadetime, 1.0
	afadeout   linseg   1.0, icrossfadetime, 0.0
	printks "------> crossfading, keeping state....\n: %f, %f", .01, afadein, afadeout
	if (k(afadein) == 1.0) then
		kactive = 0
		afadein = 1.0
		afadeout = 0.0
		printks "---->event is ended\n", .01
...
out:
	...
	instr 1	;track 1
aout 	GlitchDelay		8, .001, 3, giosc_handle, 101, "10.10.0.135", 
						"/1/fader1", "/1/rotary1", "/1/toggle1", "/1/toggle2", "/1/rotary2", "/1/rotary3", "/1/push1"
	out aout
	endin

;	instr 2	;track 1
;aout 	GlitchDelay		8, .001, .5, giosc_handle, 102, gafadein_102, gafadeout_102, "10.10.0.135", 
