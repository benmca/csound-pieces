...
void schedk_i(SCHEDK2 *p) { /* based on code by rasmus */
	O.RTevents = 1;     /* Make sure kperf() looks for RT events */
	O.ksensing = 1;
	O.OrcEvts  = 1;     /* - of the appropriate type */
}

void schedk(SCHEDK2 *p) { /* based on code by rasmus */
	int i, absinsno, argnum;
	MYFLT insno, starttime;
	EVTNODE *evtlist, *newnode;
	EVTBLK  *newevt;
...
void ButtonSched(MYFLT  *args[], int numargs) { /* based on code by rasmus */
	int i, absinsno, argnum;
	MYFLT insno, starttime;
	EVTNODE *evtlist, *newnode;
	EVTBLK  *newevt;
