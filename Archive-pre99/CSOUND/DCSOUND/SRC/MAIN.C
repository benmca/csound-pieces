...
/* start gab */
	...
	clock_t t1, t0=clock();	  
	double elapsed;
	O.inbufsamps=O.outbufsamps=800; /* gab B0 */
#define init_getstring(x,y)  
/* end gab */
	...
	extern int _8087;
	if (_8087 == 0)
		die(Str(X_490,"This Csound needs an 80x87"));
#endif
#endif
	...
	if (--argc == 0) {
        ...
        if (O.rewrt_hdr && !O.sfheader)
            dieu(Str(X_628,"can't rewrite header if no header requested"));
        ...
        if (O.filetyp == TYP_AIFF || O.filetyp == TYP_AIFC || O.filetyp == TYP_WAV) {
			if (!O.sfheader)
				dieu(Str(X_629,"can't write AIFF/WAV soundfile with no header"));
...
#else
#if !defined(SYMANTEC) && !defined(_macintosh) && !defined(LATTICE)
        VMSG(setlinebuf(stdout);)
#endif
#endif
			if (scorename == NULL) {
				if (O.RTevents) {
					err_printf(Str(X_1153,"realtime performance using dummy numeric scorefile\n"));
					if (!POLL_EVENTS()) exit(1);
					goto perf;
				}
				else {
