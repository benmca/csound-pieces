...
/*      case '-': FIND(Str(X_1043,"no listing file name")) ;
            ...
            if (!freopen(listing_file,"w",stdout))
                          die(Str(X_673,"couldn't redirect sandard out\n"));
            break;
*/
          case 'V':
    ...
    Str(X_488,"The Progress/Profile feature is currently disabled, sorry.\n"));
...
#if defined _macintosh || defined SYMANTEC || defined BCC || defined __WATCOMC__ || defined WIN32
		...
		die(Str(X_1290,"too many open files"));
          ...
          case 'F':
            ...
            O.FMidiin = 1;          /***************/
            break;
          case 'P': FIND("no pedal threshold") /* gab 18/Jan/2001 */
