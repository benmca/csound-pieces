	...
	*pt += 1;
	...
	while (!conv) {
	  ...
	  for (i=1; i<=4; i++) {
	    ...
	    f = pow(a[n1]+u*xr-m*v,2.0)+pow(u*yr,2.0);
	    if (f<fm) {
	      xm = xr;
	      ym = yr;
	      fm = f;
...
#ifdef CWIN
        {
extern int __cdecl lpanal_dialog(MYFLT *, int *, MYFLT *, MYFLT *, int *, int *,
                                 MYFLT *, MYFLT *, int *, int *, int *,
				 char **, char **);
            if (!lpanal_dialog(&sr, &channel, &beg_time, &input_dur,
			       &poleCount, &slice, &pchlow,
                               &pchhigh, &verbose, &O.displays, &storePoles,
			       &infilnam, &outfilnam)) return 0;
		...
		case '-':
		    ...
		    while (*s++); s--;
		    break;
                default: sprintf(errmsg,Str(X_1350,"unrecognized flag -%c"), *--s);
                         lpdieu(errmsg);
                }
            else break;
        } while (--argc);
   ...
   /* Do some checks on arguments we got */
	...
	coef = (MYFLT*) mmalloc((NDATA+poleCount*2)*sizeof(MYFLT));
        ...
        if (slice < poleCount * 5)
          warning(Str(X_831,"hopsize may be too small, recommend at least poleCount * 5"));
   ...
   /* Write header to disk */
	if ((nb = write(ofd,(char *)lph,(int)lph->headersize)) <
	    lph->headersize)
            quit(Str(X_630,"can't write header"));
...
/* get buffer size for one analysis frame: filtercoef or poles + freq/rms/... */
   ...
   /* Try to read first frame in buffer */
        if ((n = getsndin(infd, sigbuf, (long)WINDIN, p)) < WINDIN)
            quit(Str(X_1202,"soundfile read error, couldn't fill first frame"));
...
#if !defined(_macintosh) && !defined(CWIN)		
		exit(-1);
#else
                return(-1);
#endif
...
static void gauss(double (*a/*old*/)[MAXPOLES], double *bold, double b[])
{
    ...
    for (i=0; i < poleCount - 1;++i)  {        /* find largest pivot */
      ...
      fprintf(trace, "c[%d]=%f\n", i, c[i]);
#endif
      if (amax != a[i][i]) printf("Assumption false\n");
      for (j=i+1; j < poleCount;++j)  {             /* pivot */
        pivot = a[j][i] / amax /* a[i][i] */;
#ifdef TRACE
...
static void usage(void)
{
    ...
    err_printf(Str(X_142,
         "-d<duration>\tseconds of sound to be analyzed (default: to EOF)\n"));
