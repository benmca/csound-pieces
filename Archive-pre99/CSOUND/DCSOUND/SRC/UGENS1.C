...
void expset(EXPON *p)
{
	...
	if ((dur = *p->idur) > FZERO ) {
		...
		if ((a * b) > FZERO) {
			...
			initerror(Str(X_600,"arg1 is zero"));
		else if (b == FZERO)
			initerror(Str(X_601,"arg2 is zero"));
		else initerror(Str(X_1346,"unlike signs"));
	}
}
...
void linseg(LINSEG *p)
{
    ...
    if (p->auxch.auxp==NULL) {  /* RWD fix */
      initerror(Str(X_967,"linseg: not initialized (arate)\n"));
    }
...
void adsrset1(LINSEG *p, int midip)
{
    SEG	*segp;
    int	nsegs;
    MYFLT	**argp = p->argums;
...
void adsrset(LINSEG *p)
{
    adsrset1(p, 0);
}

void madsrset(LINSEG *p)
{
    adsrset1(p, 1);
}
...
void xsgset2(EXPSEG2 *p)  /*gab-A1 (G.Maldonado) */
{
    XSEG	*segp;
    int		nsegs;
    MYFLT	d, **argp, val, dur, nxtval;
...
/***************************************/
void expseg2(EXPSEG2 *p)		  /*gab-A1 (G.Maldonado) */
{
    XSEG	*segp;
    int		nsmps = ksmps;
    MYFLT	val, *rs; 
...
void kxpseg(EXXPSEG *p)
{
    ...
    segp = p->cursegp;
    if (p->auxch.auxp==NULL) { /* RWD fix */
      initerror(Str(X_750,"expseg (krate): not initialized"));
      return;
    }
    while (--segp->cnt < 0)
      p->cursegp = ++segp;
...
void expseg(EXXPSEG *p)
{
    ...
    segp = p->cursegp;
    if (p->auxch.auxp==NULL) { /* RWD fix */
      initerror(Str(X_749,"expseg (arate): not initialized"));
      return;
    }
    while (--segp->cnt < 0)
      p->cursegp = ++segp;
    ...
    if (p->segsrem) {           	/* done if no more segs */
      ...
      newm:
        ...
        if (segp->nxtpt == p->curval)   /*   else get new mlt   */
          p->curmlt = FONE;
        else p->curmlt = (MYFLT) pow(segp->nxtpt/p->curval, 1./segp->cnt);
      }
    ...
    if (p->segsrem) {                   /* if no more segs putk */
      ...
      newm:
        ...
        if (segp->nxtpt == val) {
          ...
          p->curmlt = (MYFLT) pow((double)(segp->nxtpt/val), 1.0/(double)segp->cnt);
          p->curamlt = (MYFLT) pow(p->curmlt,  dvensmps);
        }
      }
...
void lnrset(LINENR *p)
{
	...
	if (*p->idec > FZERO) {
		...
		p->h.insdshead->xtratim = relestim;
	    if (*p->iatdec <= FZERO)
		initerror(Str(X_1075,"non-positive iatdec"));
...
void evxset(ENVLPX *p)
{
	...
	if ((idur = *p->idur) > FZERO) {
		...
		if (iatss != FONE && (ixmod = *p->ixmod) != FZERO) {
			if (fabs(ixmod) > .95) {
				initerror(Str(X_954,"ixmod out of range."));
				return;
			}
		...
		if (!(*(ftp->ftable + ftp->flen)))
			initerror(Str(X_1168,"rise func ends with zero"));
		...
		p->mlt1 = (MYFLT) pow((double)iatss, (FONE/nk));
		if (*p->idec > FZERO) {
			if (*p->iatdec <= FZERO)
				initerror(Str(X_1075,"non-positive iatdec"));
...
void knvlpx(ENVLPX *p)
{
	...
	ftp = p->ftp;
	if (ftp==NULL) {        /* RWD fix */
          initerror(Str(X_720,"envlpx(krate): not initialized"));
          return;
	}
	...
	if ((phs = p->phs) >= 0) {
		...
		if (phs >= MAXLEN) {  /* check that 2**N+1th pnt is good */
			...
			if (!p->val)
				perferror(Str(X_719,"envlpx rise func ends with zero"));
			p->val -= p->asym;
			phs = -1L;
		}
		p->phs = phs;
...
void evrset(ENVLPR *p)
{
	...
	if (iatss != FONE && (ixmod = *p->ixmod) != FL(0.0)) {
	  if (fabs(ixmod) > .95) {
	    initerror(Str(X_954,"ixmod out of range."));
	    return;
	  }
	...
	if (*p->idec > FL(0.0)) {
	  ...
	  if ((p->rindep = (long)*p->irind))
	    ...
	    p->h.insdshead->xtratim = (short)rlscnt;
