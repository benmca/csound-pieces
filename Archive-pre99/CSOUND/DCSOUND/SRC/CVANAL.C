...
extern  char     *retfilnam;
	...
	do {
		...
		case 'd':  FIND(Str(X_1030,"no duration time"))
			   ...
			   break;
		default:   quit(Str(X_1352,"unrecognized switch option"));
	        }
	    else break;
	} while (--argc);
...
static void quit(char *msg)
{
	...
	err_printf(
	Str(X_516,"Usage: cvanal [-d<duration>] [-c<channel>] [-b<begin time>] <input soundfile> <output impulse response FFT file> \n"));
        exit(1);
}
