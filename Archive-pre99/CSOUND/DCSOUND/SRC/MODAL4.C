...
void make_Modal4(Modal4 *m, MYFLT *ifn, MYFLT vgain, MYFLT vrate)
{
    ...
    else perferror(Str(X_381,"No table for Modal4 case")); /* Expect sine wave */
...
void marimbaset(MARIMBA *p)
{
    ...
    else perferror(Str(X_380,"No table for Marimba strike")); /* Expect an impulslything */
...
void vibraphnset(VIBRAPHN *p)
{
    ...
    if ((ftp = ftfind(p->ifn)) != NULL)	p->m4.wave = ftp;
    else perferror(Str(X_385,"No table for Vibraphone strike")); /* Expect an impulslything */
...
void agogobelset(VIBRAPHN *p)
{
    ...
    if ((ftp = ftfind(p->ifn)) != NULL)	p->m4.wave = ftp;
    else perferror(Str(X_374,"No table for Agogobell strike")); /* Expect an impulslything */
