...
static char *prevp(SRTBLK *bp, char *p)
{
    ...
    error:
      err_printf(
                 Str(X_1259,"swrite: output, sect%d line%d p%d makes illegal reference to "),
                 sectcnt,lincnt,pcnt);
      while (q < p)
        err_printf("%c", *q++);
      while (*p != SP && *p != LF)
...
extern  MYFLT stof(char*);
 ...
 error1:
    err_printf(Str(X_1257,"swrite: output, sect%d line%d p%d has illegal ramp symbol\n"),
               sectcnt,lincnt,pcnt);
    goto put0;
 error2:
...
/*  printf("rval=%f bp->newp2=%f prvbp->newp2-%f\n", rval, bp->newp2, prvbp->newp2); */
 ...
 error1:
    err_printf(Str(X_1255,"swrite: output, sect%d line%d p%d has illegal expramp symbol\n"),
	       sectcnt,lincnt,pcnt);
    goto put0;
 error2:
