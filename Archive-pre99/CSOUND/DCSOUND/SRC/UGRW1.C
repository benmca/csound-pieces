 ...
 * it, catch negative values too.  */
    if (*p->xfn < 1) {
      sprintf(errmsg, Str(X_485,"Table write k rate function table no. %f < 1"), *p->xfn);
      perferror(errmsg);
      return (0);
    }
...
{
    printf(Str(X_949,"itablegpw is a deprecated opcode -- please use tableigpw\n"));
    itablegpw(p);
}
...
void itablemix(TABLEMIX *p)
{
    ...
    if ((*p->dft < 1) || (*p->s1ft < 1) || (*p->s2ft < 1)) {
	sprintf(errmsg, Str(X_484,"Table number < 1 dft=%.2f  s1ft=%.2f  s2ft=%.2f\n"),
		*p->dft, *p->s1ft, *p->s2ft);
	initerror(errmsg);
	return;
    }
...
{
    printf(Str(X_950,"itablemix is a deprecated opcode -- please use tableimix\n"));
    itablemix(p);
}
...
{
    printf(Str(X_948,"itablecopy is a deprecated opcode -- please use tableicopy\n"));
    itablecopy(p);
}
...
void tablera(TABLERA *p)
{
	...
	if (p->ftp->flen < ksmps) {
	  sprintf(errmsg, Str(X_481,"Table kfn=%.2f length %ld shorter than ksmps %d\n"),
    ...
    if (((kstart = (long)*p->kstart) < 0L) || (kstart >= p->ftp->flen)) {
      sprintf(errmsg, Str(X_961,"kstart %.2f is outside table %.2f range 0 to %ld\n"),
...
void zkr(ZKR *p)
{
    ...
    indx = (long) *p->ndx;
    if (indx > zklast) {
      perferror(Str(X_1411,"zkr index > isizek. Returning 0."));
      *p->rslt = FL(0.0);
    }
    else if (indx < 0) {
      perferror(Str(X_1410,"zkr index < 0. Returning 0."));
      *p->rslt = FL(0.0);
    }
    else {
...
void printksset(PRINTKS *p)
{
	...
	sprintf(errmsg, Str(X_1136,"printks parm 1 was not a \"quoted string\"\n"));
	initerror(errmsg);
	return;
    }
    else {
