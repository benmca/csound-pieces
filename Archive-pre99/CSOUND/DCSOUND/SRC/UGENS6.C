...
static MYFLT	log001 = -FL(6.9078);	/* log(.001) */
...
void delayr(DELAYR *p)
{
    ...
    if (p->auxch.auxp==NULL) { /* RWD fix */
      initerror(Str(X_688,"delayr: not initialized"));
      return;
    }
    ar = p->ar;
    curp = p->curp;
...
void delayw(DELAYW *p)
{
    ...
    if (q->auxch.auxp==NULL) { /* RWD fix */
      initerror(Str(X_689,"delayw: not initialized"));
      return;
    }
    asig = p->asig;
    curp = q->curp;
...
void deltap(DELTAP *p)
{
    ...
    if (q->auxch.auxp==NULL) { /* RWD fix */
      initerror(Str(X_691,"deltap: not initialized"));
      return;
    }
    ar = p->ar;
...
void deltapi(DELTAP *p)
{
    ...
    if (q->auxch.auxp==NULL) {
      initerror(Str(X_692,"deltapi: not initialized"));
      return;
    }
    ar = p->ar;
...
void deltapn(DELTAP *p)
{
  ...
  if (q->auxch.auxp==NULL) {
    initerror(Str(X_693,"deltapn: not initialized"));
    return;
  }
  ar = p->ar;
  begp = (MYFLT *) q->auxch.auxp;
...
void deltap3(DELTAP *p)
{
    ...
    if (q->auxch.auxp==NULL) {
      initerror(Str(X_690,"deltap3: not initialized"));
      return;
    }
    ar = p->ar;
    begp = (MYFLT *) q->auxch.auxp;
    ...
    if (!p->XINCODE) {
      ...
      do {
        ...
        y0 = *tap; y1 = *prv;
        {
          ...
          MYFLT t1 = y2 + 3*y0;
          *ar++ = y0 + FL(0.5)*frcu +
            delfrac*(y1 - frcu/FL(6.0) - t1/FL(6.0) - ym1/FL(3.0)) +
            frsq*delfrac*(t1/FL(6.0) - FL(0.5)*y1) + frsq*(FL(0.5)* y1 - y0);
        }
        tap++;
      }
      while (--nsmps);
...
void comb(COMB *p)
{
    ...
    if (p->auxch.auxp==NULL) { /* RWD fix */
      initerror(Str(X_665,"comb: not initialized"));
      return;
    }
    if (p->prvt != *p->krvt) {
#ifdef __alpha__
...
void alpass(COMB *p)
{
    ...
    if (p->auxch.auxp==NULL) { /* RWD fix */
      initerror(Str(X_595,"allpadd: not initialized"));
      return;
    }
    if (p->prvt != *p->krvt) {
      coef = p->coef = (MYFLT)exp((double)(log001 * *p->ilpt / *p->krvt));
...
void reverb(REVERB *p)
{
    ...
    if (p->auxch.auxp==NULL) { /* RWD fix */
      initerror(Str(X_1166,"reverb: not intialized"));
      return;
    }
    if (p->prvt != *p->krvt) {
      MYFLT	*lptimp = revlptimes;
...
void pan(PAN *p)
{
    ...
    ftp = p->ftp;
    if (ftp==NULL) {        /* RWD fix */
      initerror(Str(X_1118,"pan: not initialized"));
      return;
    }
    flen = ftp->flen;
