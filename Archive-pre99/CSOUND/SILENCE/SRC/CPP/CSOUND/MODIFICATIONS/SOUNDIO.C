...
static void floatran_noscl(void); /* I.Varga */
void iotranset(void)
...
#ifdef WIN32
      extern int win_dev;
      /*win_dev = 0;*/     /*RWD Nov 2001: leave it at 1024 init */
 ...
 nchk:
    ...
    do {
      ...
      if (fltsmp >= 0) { /* +ive samp:   */
        if (fltsmp > 32768.0) {          /* out of range?     */
          /*   report it*/
          rngp = rngcnt + (maxampp - maxamp);
          (*rngp)++;
          rngflg = 1;
...
#if defined(mac_classic) || defined(SGI) || defined(LINUX_BE)
