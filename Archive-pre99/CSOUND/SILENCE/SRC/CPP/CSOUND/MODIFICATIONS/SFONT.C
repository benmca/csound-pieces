/* WARNING! This file MUST be compiled by setting the structure member
   ...
   should be present beetween a structure data member and another.
...
void SoundFontLoad(char *fname)
{
    FILE *fil;
    char pathnam[255];
    extern char *ssdirpath, *sfdirpath;
...
#define errno _errno
      ...
      if (!isfullpath(fname)) {
        if (ssdirpath != NULL) {
          ...
          if ((access(pathnam, 0 )) != -1 ) {/* if file exists */
            if (access(pathnam, 0x6))   /* check its access state */
              chmod(pathnam, S_IREAD | S_IWRITE); /* ... and force it rw */
          }
#endif
#endif
        ...
        if (sfdirpath != NULL) {
          ...
          if ((access(pathnam, 0 )) != -1 ) {/* if file exists */
            if (access(pathnam, 0x6))   /* check its access state */
              chmod(pathnam, S_IREAD | S_IWRITE);
          }
#endif
#endif
          if (( fil = fopen(pathnam,"rb")) != NULL) goto done;
...
void SfPreset(SFPRESET *p)
{
    ...
    *p->ipresethandle = (float) presetHandle;
      ...
      if (notnum >= layer->minNoteRange && notnum <= layer->maxNoteRange &&
        ...
        for (k = 0; k < splitsNum; k++) {
          ...
          if (notnum >= split->minNoteRange && notnum  <= split->maxNoteRange &&
            ...
            attenuation = (MYFLT) pow(2.0, (-1.0/60.0) * attenuation ) * GLOBAL_ATTENUATION;
            pan = (double)(split->pan + layer->pan) / 1000.0 + 0.5;
            if (pan > 1.0) pan = 1.0;
            else if (pan < 0.0) pan = 0.0;
            p->base[spltNum] = sBase+ start;
            p->end[spltNum] = sample->dwEnd - start;
            p->startloop[spltNum] = sample->dwStartloop - start;
            p->endloop[spltNum] = sample->dwEndloop - start;
          ...
          if (notnum >= split->minNoteRange &&
            ...
            p->attenuation[spltNum] =
              (MYFLT) pow(2.0, (-1.0/60.0) * (layer->initialAttenuation +
                                              split->initialAttenuation)) * GLOBAL_ATTENUATION;
            p->base[spltNum] =  sBase+ start;
            p->phs[spltNum] = (double) split->startOffset;
...
void SfInstrPlayMono_set(SFIPLAYMONO *p)
{
    ...
    SFBANK *sf = &sfArray[index];
    if (index > currSFndx || *p->instrNum >  sf->instrs_num) {
      initerror(Str(X_1500,"sfinstr: instrument out of range"));
      return;
    }
    else {
      instrType *layer = &sf->instr[(int) *p->instrNum];
      ...
      for (k = 0; k < splitsNum; k++) {
        ...
        if (notnum >= split->minNoteRange &&
          ...
          p->attenuation[spltNum] = (MYFLT) pow(2.0, (-1.0/60.0) * split->initialAttenuation) * GLOBAL_ATTENUATION;
          p->base[spltNum] = sBase+ start;
          p->phs[spltNum] = (double) split->startOffset;
          p->end[spltNum] = sample->dwEnd + split->endOffset - start;
          p->startloop[spltNum] = sample->dwStartloop + split->startLoopOffset - start;
 ...
 */
#if defined(mac_classic) || defined(SGI)
static void
        ...
        if (*chkid == s2d("INFO")) {
          ...
          do {
            ...
            if (*chkid == s2d("phdr")) {
              ...
              ChangeByteOrder("d", chkp + 4, 4);
              ChangeByteOrder("w5", chkp + 8, pbagChunk->ckSize);
              ...
              ChangeByteOrder("d", chkp + 4, 4);
              ChangeByteOrder("b20w", chkp + 8, pbagChunk->ckSize);
              ...
              ChangeByteOrder("d", chkp + 4, 4);
              ChangeByteOrder("w2", chkp + 8, imodChunk->ckSize);
