...
void fftmx(MYFLT *,MYFLT *,int,int,int,int,int,int *,MYFLT *,MYFLT *,MYFLT *,MYFLT *,int *,int[]);
void reals_(MYFLT *,MYFLT *,int,int);
void pvx_release(PVX **ppvx,long chans);
static long generate_frame(PVX *pvx,MYFLT *fbuf,float *outanal,long samps,int frametype);
static void chan_split(const MYFLT *inbuf,MYFLT **chbuf,long insize,long chans);
static int init(PVX **pvx,long srate,long fftsize,long winsize,long overlap,pv_wtype wintype);
...
int pvxanal(SOUNDIN *p,int fd,const char *fname,long srate,long chans,long fftsize,
            long overlap, long winsize,pv_wtype wintype,int verbose)
{
    ...
    for (i=0;i < chans;i++) {
      ...
      frame_c[i] = (float*)malloc((fftsize+2)*sizeof(float)); /*RWD MUST be 32bit*/
      if (frame_c[i]==NULL) {
        rc = 1;
        goto error;
      }
         ...
         long overlap,pv_wtype wintype)
{
    ...
    thispvx->beta = 6.8f;       /* parameter for Kaiser window */
    ...
    if (M > N) {
      if (Mf)
        *thispvx->analWindow *=
          (MYFLT)((double)N*sin((double)PI*0.5/N)/(double)(PI*0.5));
      for (i = 1; i <= thispvx->analWinLen; i++)
    ...
    thispvx->invR =( 1.0f / thispvx->R);
    thispvx->RoverTwoPi = (MYFLT)(thispvx->rIn / TWOPI);
...
void vonhann(MYFLT *win,int winLen,int even)
{
    ...
    if (even) {
      for (i=0; i<winLen; i++)
        *(win+i) = (MYFLT)(.5 + .5 *cos(ftmp*((double)i+.5)));
      *(win+winLen) = FL(0.0);
    }
    else{       *(win) = FL(1.0);
    for (i=1; i<=winLen; i++)
      *(win+i) =(MYFLT)(.5 + .5 *cos(ftmp*(double)i));
    }
}
