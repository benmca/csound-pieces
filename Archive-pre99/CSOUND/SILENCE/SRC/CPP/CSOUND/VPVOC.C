...
void ktableseg(TABLESEG *p)
{
    ...
    if (p->auxch.auxp==NULL) {
      initerror(Str(X_1270,"tableseg: not initialised"));
      return;
    }
    segp = p->cursegp;
...
void ktablexseg(TABLESEG *p)
{
    ...
    /* RWD fix */
    if (p->auxch.auxp==NULL) {
      initerror(Str(X_1271,"tablexseg: not initialised"));
      return;
    }
    segp = p->cursegp;
    curtab = segp->function->ftable;
...
void voscili(VOSC *p)
{
    ...
    if (q->auxch.auxp==NULL) {
      initerror(Str(X_1373,"voscili: not initialised"));
      return;
    }
    ftp = q->outfunc;
    ftab = ftp->ftable;
...
/*     MYFLT  *samp; */
    ...
    /* RWD fix */
    if (p->auxch.auxp==NULL) {
      perferror(Str(X_1374,"vpvoc: not initialised"));
       return;
    }
