...
void vbap_FOUR_moving_control(VBAP_FOUR_MOVING *);

void vbap_FOUR(VBAP_FOUR  *p)   /* during note performance:   */
{
...
void vbap_FOUR_control(VBAP_FOUR  *p)
{
...
void vbap_FOUR_init(VBAP_FOUR  *p)
...
void vbap_FOUR_moving(VBAP_FOUR_MOVING  *p) /* during note performance:   */
{
    ...
    /* write audio to resulting audio streams wFOURed
       with gain factors*/
    invfloatn =  FL(1.0) / (MYFLT) ksmps;
    for (j=0; j<FOUR ;j++) {
      inptr = p->audio;
...
void vbap_FOUR_moving_control(VBAP_FOUR_MOVING *p)
{
    CART_VEC spreaddir[16];
    CART_VEC spreadbase[16];
    ANG_VEC atmp;
    ...
    *p->spread=FL(0.0);
    ...
    if (p->point_change_counter++ >= p->point_change_interval) {
      ...
      if (p->dim == 3) { /*jumping over second field */
        p->curr_fld = p->next_fld;
        if (++p->next_fld >= ((int) fabs(*p->field_am))) {
          if (*p->field_am >= 0.0) /* point-to-point */
            p->next_fld = 0;
          else
            p->next_fld = 1;
        }
      ...
      if ((p->fld[abs(p->next_fld)]==NULL))
        die("Missing fields in vbap4move\n");
...
void vbap_FOUR_moving_init(VBAP_FOUR_MOVING  *p)
{
    int i,j;
    MYFLT *ptr;
    LS_SET *ls_set_ptr;
    ...
    if (fabs(*p->field_am) < (2+ (p->dim - 2)*2)) {
      printf("Have to have at least %d directions in vbap4move\n",2+ (p->dim - 2)*2);
      exit(-1);
    }
    if (p->dim == 2)
