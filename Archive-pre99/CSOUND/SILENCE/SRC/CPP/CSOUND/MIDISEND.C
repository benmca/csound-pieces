...
extern void MacMidiOutShortMsg(unsigned char *data);
...
void openMIDIout(void)
{
  ...
  if (serverPort >= B_OK) {
    ...
    if (gMidiOutPort >= B_OK) {
      ...
      status = write_port(serverPort, kServerNewMidiOutPort, &request, sizeof(request));
      if (status >= B_NO_ERROR) {
        /* Send name of MIDI output device. */
        if (O.Midioutname) {
          status = write_port(gMidiOutPort, 0, O.Midioutname, strlen(O.Midioutname));
        } else {
  ...
  if (gMidiOutPort < B_OK) {
    err_printf(Str(X_753,"Failed to open MIDI output port to Csound Server.\n"));
  }
  MIDIoutDONE = 1;
}
