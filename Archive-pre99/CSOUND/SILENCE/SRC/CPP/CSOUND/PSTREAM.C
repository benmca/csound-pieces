...
void fassign(FASSIGN *p)
{
    int i;
    long framesize;
    float *fout,*fsrc;
    if (!fsigs_equal(p->fout,p->fsrc))
      die(Str(X_1572,"fsig = : formats are different.\n"));
...
void pvadsynset(PVADS *p)
{
    ...
    p->outptr = 0;
    p->lastframe = 0;
    p->one_over_sr = (float)(FL(1.0) /  esr);
    p->pi_over_sr = (float)(PI * p->one_over_sr);
...
static void adsyn_frame(PVADS *p)
{
    ...
    memset(p->outbuf.auxp,0,p->overlap * sizeof(float)); /* RWD must be 32bit */
    ...
    for (i=startbin;i < lastbin;i+= binoffset) {
      ...
      if (freqs[i] > nyquist)
        amps[i] = FL(0.0);
      a[i] = (MYFLT )(2.0 * sin( freqs[i] * p->pi_over_sr));
    }
...
void pvsmaska(PVSMASKA *p)
{
    ...
    if (depth < FL(0.0)) {
      ...
      if (!p->nwarned)  {
        warning(Str(X_1592,"pvsmaska: negative value for kdepth; clipped to zero.\n"));
        p->nwarned = 1;
      }
      depth = FL(0.0);
    }
    if (depth > 1.0f) {
      if (!p->pwarned)  {
        warning(Str(X_1593,"pvsmaska: kdepth > 1: clipped.\n"));
        p->pwarned = 1;
      }
      depth = FL(1.0);
    }
...
static int pvx_loadfile(const char *fname,PVSFREAD *p,MEMFIL **mfp)
{
    ...
    if ((p->arate = (MYFLT) fmt.nSamplesPerSec) != esr) { /* & chk the data */
