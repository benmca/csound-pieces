...
void grain2 (GRAIN2 *p)
{
    ...
    if (p->nr_osc == -1) {
      ...
      initerror (Str(X_1668,"grain2: not initialised")); return;
    }
...
void grain3 (GRAIN3 *p)
{
    ...
    if ((p->seed == 0L) || (p->osc == NULL)) {
      initerror (Str(X_1669,"grain3: not initialised")); return;
    }
    ...
    if ((w_frq_f < (FL(1.0) / (MYFLT) OSCBNK_PHSMAX)) ||
        (w_frq_f >= FL(1.0))) {
      perferror (Str(X_1670,"grain3: invalid grain duration"));
      return;
    }
    ...
    if ((x_frq_f < (FL(1.0) / (MYFLT) OSCBNK_PHSMAX)) ||
        (x_frq_f >= FL(1.0))) {
      perferror (Str(X_1671,"grain3: invalid grain density"));
      return;
    }
    ...
    if ((p->init_k) && (!(p->mode & 0x10))) {
      ...
      while (g_ph) {
        ...
        if (p->osc_end == p->osc_start) {
          perferror (Str(X_1672,"grain3 needs more overlaps"));
          return;
        }
        g_ph -= g_frq;
      }
...
void rnd31k (RND31 *p)
{
    ...
    if ((p->seed < 1L) || (p->seed > 0x7FFFFFFEL)) {
      initerror (Str(X_1673,"rnd31: not initialised")); return;
    }
