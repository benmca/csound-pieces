...
void vdelset(VDEL *p)           /*  vdelay set-up   */
{
    ...
    /* Not needed as always+1 if (n == 0) n = 1;	/* fix due to Troxler */
...
void vdelay(VDEL *p)              /*      vdelay  routine */
{
    ...
    if (buf==NULL) {            /* RWD fix */
      initerror(Str(X_1372,"vdelay: not initialised"));
      return;
    }
...
void vdelay3(VDEL *p)              /*      vdelay routine with cubic interp */
{
    ...
    if (buf==NULL) {            /* RWD fix */
      initerror(Str(X_1371,"vdelay3: not initialised"));
      return;
    }
...
void vdelayx(VDELX *p)              /*      vdelayx routine  */
{
    ...
    if (buf1 == NULL) {
      initerror(Str(X_1372,"vdelay: not initialised"));		/* RWD fix */
      return;
    }
...
{                               /* assign object data to local variables   */
    ...
    if (buf==NULL) {            /* RWD fix */
      initerror(Str(X_1008,"multitap: not initialised"));
      return;
    }
    do {
...
void nreverb(NREV *p)
{
    ...
    if (p->temp.auxp==NULL) {
      initerror(Str(X_1165,"reverb2: not initialised"));
      return;
    }
    do  *out++ = FL(0.0);
    while (--n);
...
void reverbx_set(NREV2 *p)
{
	...
	p->c_gain[i] = (MYFLT)exp((double)(LOG001 * (p->c_time[i]/esr) /
      ...
      for (i = 0 ; i < p->numCombs ; i++) {
        p->pcbuf_cur[i+1] = p->cbuf_cur[i+1] = p->cbuf_cur[i] + (int)p->c_time[i];
        p->c_time[i] /= esr;    /* Scale to save division in reverbx */
      }
      n = 0;
      ...
      for (i = 0; i < p->numAlpas ; i++) {
        p->pabuf_cur[i+1] = p->abuf_cur[i+1] = p->abuf_cur[i] + (int)p->a_time[i];
        p->a_time[i] /= esr;    /* Scale to save division in reverbx */
      }
    }
...
void reverbx(NREV2 *p)
{
    ...
    if (p->temp.auxp==NULL) {
      initerror(Str(X_1546,"reverbx: not initialised"));
      return;
    }
