...
MYFLT cpu_power_busy=FL(0.0);  /* accumulates the supposed percent of cpu usage */
extern  INSTRTXT **instrtxtp;
extern int      maxinsno;
...
void adsynt(ADSYNT *p)
{
    ...
    if (p->inerr) {
      initerror(Str(X_1475,"adsynt: not initialised"));
      return;
    }
    ftp = p->ftp;
    ftbl = ftp->ftable;
...
void hsboscil(HSBOSC  *p)
{
    ...
    mixtp = p->mixtp;
    if (ftp==NULL) {
      initerror(Str(X_1476,"hsboscil: not initialised"));
      return;
    }
    if (mixtp==NULL) {
      initerror(Str(X_1476,"hsboscil: not initialised"));
...
void pitchamdfset(PITCHAMDF *p)
{
    ...
    if (p->medisize) {
      ...
      if (p->median.auxp==NULL || p->median.size < (long)sizeof(MYFLT)*msize)
        auxalloc(sizeof(MYFLT)*(msize), &p->median);
    ...
    if (p->buffer.auxp==NULL || p->buffer.size < (long)sizeof(MYFLT)*(bufsize)) {
      auxalloc(sizeof(MYFLT)*(bufsize), &p->buffer);
...
void pitchamdf(PITCHAMDF *p)
{
    ...
    if (p->inerr) {
      initerror(Str(X_1478,"pitchamdf: not initialised"));
      return;
    }
...
void kphsorbnk(PHSORBNK *p)
{
    ...
    if (curphs == NULL) {
      initerror(Str(X_1479,"phasorbnk: not initialised"));
      return;
    }
...
void GardnerPink_init(PINKISH *p);
void GardnerPink_perf(PINKISH *p);


void pinkset(PINKISH *p)
{
        /* Check valid method */
    if (*p->imethod != GARDNER_PINK && *p->imethod != KELLET_PINK
        && *p->imethod != KELLET_CHEAP_PINK)
      initerror(Str(X_1533,"pinkish: Invalid method code"));
    /* User range scaling can be a- or k-rate for Gardner, a-rate only for filter */
    if (p->XINCODE & 0x02) {
      p->ampinc = 1;
    }
    else {
...
void Foscset(XOSC *p)
{
    ...
    if ((ftp = ftnp2find(p->ifn)) != NULL) { /* Allow any length */
      p->ftp = ftp;
      if (*p->iphs >= 0)
        p->lphs = ((unsigned long)(*p->iphs * fmaxlen));
    }
}
...
void isense(KSENSE *p)
{
    setvbuf(stdin, NULL, _IONBF, 0);
}

void ksense(KSENSE *p)
{
    fd_set rfds;
    struct timeval tv;
    int retval;
...
void impulse_set(IMPULSE *p)
{
    p->next = (int)(FL(0.5) + *p->offset * esr);
}

void impulse(IMPULSE *p)
{
    int nsmps = ksmps;
    int next = p->next;
    MYFLT *ar = p->ar;
...
/*        printf("%f: cnt=%ld alpha=%f c1=%f nxt=%f\n", */
/*               dur, segp->cnt, alpha, segp->c1, segp->nxtpt); */
      segp->alpha = alpha/d;
      val = nxtval;
      segp++;
    } while (--nsegs);
...
/*          printf("%d: cnt=%ld alpha=%f const=%f, nxt=%f\n", i, */
/*                 ((NSEG*)p->auxch.auxp)[i].cnt, */
/*                 ((NSEG*)p->auxch.auxp)[i].alpha, */
/*                 ((NSEG*)p->auxch.auxp)[i].c1, */
/*                 ((NSEG*)p->auxch.auxp)[i].nxtpt); */
/*      } */
}

void ktrnseg(TRANSEG *p)
{
...
void trnseg(TRANSEG *p)
{
    MYFLT  val, *rs = p->rslt;
    int         nsmps = ksmps;
    NSEG        *segp = p->cursegp;
    if (p->auxch.auxp==NULL) {
      initerror(Str(X_1553, "transeg: not initialised (arate)\n"));
    }
...
/*          printf("New segment: "); */
      ...
      chk1:
        ...
        p->alpha = segp->alpha;
        p->curx = FL(0.0);
/*          printf("const = %f  alpha=%f val=%f(%f)\n", */
/*                 p->curinc, p->alpha, val, segp->val); */
        p->curval = val;
      }
      if (p->alpha == FL(0.0)) {
        do {
...
void lpf18db(LPF18 *p)
{
    int         nsmps = ksmps;
    MYFLT kfcn = FL(2.0)**p->fco/(esr);
...
/*         printf("Sign change %g %g at %d\n", p->lastsamp, samp, index); */
          p->direction = 1;
          if (++p->cnt > *p->rep) {
/*             printf("Count expired\n"); */
            p->cnt = 1;
            p->start = index;
            p->noinsert = 0;
          }
          else {
/*             printf("Count %d index now %d\n", p->cnt, p->start); */
            index = p->start;
            insert = (MYFLT*)(p->auxch.auxp) + index;
          }
        }
