 ...
 * v1.01 Richard W Dobson September 26 2000  */


/* updates:
 * 26:9:2000   deal with Ircam 1NVT frames, and handle other skipped
 * frames properly
*/
...
void usage(void)
{
    printf("SDIF2ADS v1.01: convert sdif 1TRC data to Csound hetro data file.\n"
           ...
           "      of 1024 partials may not be realised in all cases.\n");
}
...
/* if anyone wants it, there is enough information to select a frequency
 * range to output.*/
...
{
    int i,framecount,total_partials = 0,partials_written = 0;
    int result, iindex;
    ...
    float thistime;
    float maxfreq = 0.0f, minfreq = 100000.0f;
    float maxamp = 0.0f, frame_maxamp = 0.0f;
    float scalefac = 1.0f;
    int doscale = 0;
    ...
    while (argv[1][0]=='-') {
      ...
      case('p'):
        ...
        break;
      default:
        fprintf(stderr,"\nunrecognised flag option %s",argv[1]);
        usage();
        exit(1);
      }
      argc--;
    ...
    if (argc < 2) {
      fprintf(stderr,"\nSDIF2ADS: insufficient arguments");
      usage();
      exit(1);
    }
     ...
     * we start a new partial in the array, at the given index.
     * When a partial dies, the amplitude is (presumed to be) zero,
     * but the list is not terminated.   If an SDIF frame with that
     * index starts a new partial, the partial with that index
     ...
     *  MAXPARTIALS, we are b******d. Of course, the SDIF file will
     *  not tell us what the index range is. That would be too easy.
     */
    ...
    while ((result = SDIF_ReadFrameHeader(&fh, infile))==ESDIF_SUCCESS) {
      if (strncmp(fh.frameType,"1TRC",4)) {
        /* could be frame such as 1NVT, with no matrices, but text data,
           so must always skip data explicitly */
        if (SDIF_SkipFrame(&fh,infile) != ESDIF_SUCCESS) {
          fprintf(stderr,"Error skipping frame.\n");
          exit(1);
        }
        continue;
      }
      ...
      /* Csound adsyn counts time in msecs, in shorts, so can only
       * have 32.767 seconds! */
      if (thistime > 32.76f) {
      ...
      for (i = 0; i < fh.matrixCount; ++i) {
        ...
        for (j=0;j < n_partials; j++) {
          ...
          iindex = (int) index;
                                /* a small (unlikely) sacrifice for
                                   the sake of a simple algorithm! */
